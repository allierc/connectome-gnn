...
Verbatim port of Yang et al. 2019's gyyang/multitask task generator
(`papers/multi-tasks/src/NeuralGraph/generators/task.py`). Python-3-only;
`six.string_types` replaced with `str` and `rule is 'testinit'` replaced
with `rule == 'testinit'` so the file works without the `six` shim.
"""
...
def get_default_hp(ruleset):
    ...
    Returns:
        hp : a dictionary containing training configuration
    '''
