...
class DrosophilaCxVoltageODEParams(ODEParamsBase):
    ...
    the CX voltage dataset is generated by rolling the no-TV cv0 RNN teacher and
    saving the graded-voltage schema (``tau_i, V_i_rest, edge_index, signed W``),
    so the on-disk fields match FlyVis. It is a *separate* model, though — the
    teacher's nonlinearity is a softplus (beta=5), not ReLU — so it gets its own
    g_phi reference curve. The per-neuron gain/bias (g, b) are not saved with the
    voltage rollout, so the GT g_phi is the shared softplus shape; the per-neuron
    W correction uses the base linear-slope fit on the learned curves.
    ...
    def gt_g_phi_func(self, v):
        """GT g_phi shape = softplus(v, beta=5), the CX teacher's nonlinearity.
        Per-neuron gain/bias (g, b) are not carried by the voltage rollout, so
        this is the shared curve. Same shape as ``v`` ((n_pts,) or (N, n_pts))."""
        beta = 5.0
        return np.where(beta * v > 20, v, np.log1p(np.exp(beta * v)) / beta)

    def g_phi_label(self):
        return r"$\mathrm{softplus}(v_j, \beta\!=\!5)$"
