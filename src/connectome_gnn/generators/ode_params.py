...
Usage:
    ...
    class FlyVisCurrentODEParams(ODEParamsBase):
        ...
...
class ODEParamsBase:
    ...
    #: Which generator wrote these parameters. Saved into ode_params.pt so a
    #: dataset says what it is -- otherwise nothing on disk distinguishes
    #: current-generated from conductance-generated data, and every reader has
    #: to be told out of band. Subclasses override it.
    GROUND_TRUTH_MODEL: ClassVar[str] = "current"

    def save(self, folder: str):
        """Save all fields as a single ode_params.pt dict."""
        os.makedirs(folder, exist_ok=True)
        state = {"ground_truth_model": self.GROUND_TRUTH_MODEL}
        for f in dc_fields(self):
            val = getattr(self, f.name)
            if isinstance(val, torch.Tensor):
    ...
    def load(cls, folder: str, device: torch.device | str = "cpu"):
        ...
        if os.path.exists(unified_path):
            ...
            # Written by save(); not a dataclass field. Files predating it have
            # no key and are current-generated by definition.
            state.pop("ground_truth_model", None)
            return cls(**state)
...
# ---------------------------------------------------------------------------
# Drosophila CX voltage-rollout params (Branch 0 — voltage system ID)
# ---------------------------------------------------------------------------


# Old name, kept so any script or notebook that imports it keeps working. It is
# the SAME object, not a subclass: `FlyVisODEParams is FlyVisCurrentODEParams`.
# The rename says which synapse model the parameters describe, now that a
# conductance-based one exists beside it (FlyVisConductanceODEParams).
FlyVisODEParams = FlyVisCurrentODEParams


def load_flyvis_ode_params(folder: str, device: torch.device | str = "cpu"):
    """Load a flyvis dataset's parameters as whatever class wrote them.

    `ODEParamsBase.load` builds the class you ask for; this asks the FILE. A
    conductance dataset carries E_exc/E_inh/edge_is_inh that the current class
    has no fields for, so loading it as the wrong class raises rather than
    silently dropping the reversals -- but only if the caller guessed. Use this
    when the caller does not already know which kind of dataset it holds.
    """
    path = os.path.join(folder, "ode_params.pt")
    if not os.path.exists(path):
        return FlyVisCurrentODEParams.load(folder, device)      # legacy per-file
    state = torch.load(path, map_location="cpu", weights_only=True)
    kind = state.get("ground_truth_model", "current")
    cls = {"current": FlyVisCurrentODEParams,
...
@dataclass
class FlyVisConductanceODEParams(FlyVisCurrentODEParams):
    """Parameters for the CONDUCTANCE-based FlyVis ODE.

        msg_ij  = W_ij * relu(v_j) * (E_i - v_i)
        dv_i/dt = (-v_i + sum_j msg_ij + I_i + V_rest_i) / tau_i

    NOT in the ODE-params registry. That registry is keyed on the model that
    will be TRAINED on a dataset, and no one trains "the conductance generator";
    `simulation.ground_truth_model` selects this class directly. `ode_params.pt`
    records which generator wrote it, so a dataset says what it is.

    W IS A CONDUCTANCE, NOT A SIGNED WEIGHT -- this is the field whose meaning
    changes, and the one thing to check before reusing any code that reads
    `ode_params.W`. On the current-based class W is
    `syn_strength * syn_count * sign` and carries the synapse's sign; here it
    is non-negative and the sign lives entirely in the driving force
    `(E_i - v_i)`, selected per edge by `edge_is_inh`. Anything that infers
    excitation/inhibition from sign(W) -- effective_true_weights, the
    plot_synaptic W-correction, the Dale and coeff_W_sign regularisers -- is
    meaningless on these datasets.

    Added fields:
        E_exc:       (N,) excitatory reversal potential, per POSTSYNAPTIC neuron
        E_inh:       (N,) inhibitory reversal potential, per POSTSYNAPTIC neuron
        edge_is_inh: (E,) bool, the presynaptic cell's Dale sign

    E is stored per neuron even when it was learned globally or per cell type.
    The driving force is (E - v_i), so E belongs to the postsynaptic cell, and
    expanding once here means the generator never has to know which
    granularity produced it.
    """

    GROUND_TRUTH_MODEL: ClassVar[str] = "conductance"

    E_exc: torch.Tensor = None        # (N,)
    E_inh: torch.Tensor = None        # (N,)
    ...
    def from_twin_checkpoint(cls, ckpt_path, edge_index, n_neurons=None,
        ...
        The student stores what it needed to TRAIN; a generator needs it in the
        teacher's layout. Three conversions, each asserted rather than assumed:

          W        (E, 1) raw  ->  (E,) SQUARED. The model squares at use to
                   keep the conductance non-negative, so the stored parameter
                   is the square root and the dataset must carry the square.
          raw_tau  (65,)      ->  softplus, then expanded through `type_index`
                   to (N,). The student parameterises tau and V_rest PER CELL
                   TYPE because the teacher genuinely has 65 distinct values
                   over 13741 neurons; the ODE indexes them per neuron.
          E        (1,) | (65,) | (N,)  ->  (N,), by the same rule.

        `edge_index` is not in the checkpoint -- it is a property of the
        connectome, not of the fit -- so it must be supplied and is checked
        against the number of edges the student was trained on.
        """
        raw = torch.load(ckpt_path, map_location="cpu", weights_only=False)
        sd = raw.get("model_state_dict", raw) if isinstance(raw, dict) else raw
        sd = {k.replace("_orig_mod.", ""): v for k, v in sd.items()}  # torch.compile

        missing = [k for k in ("W", "raw_tau", "V_rest", "E_exc", "E_inh",
                               "type_index", "edge_is_inh") if k not in sd]
        if missing:
            raise KeyError(
                f"{ckpt_path} is missing {missing}; is it a "
                f"flyvis_conductance_known_ode checkpoint?")

        type_index = sd["type_index"].long()
        n = int(n_neurons or type_index.numel())
        if type_index.numel() != n:
            raise ValueError(
                f"type_index has {type_index.numel()} entries but n_neurons={n}")

        def _per_neuron(v, what):
            v = torch.as_tensor(v).float().flatten()
            if v.numel() == n:
                return v
            if v.numel() == 1:
                return v.expand(n).clone()
            if v.numel() == int(type_index.max()) + 1:
                return v[type_index].clone()
            raise ValueError(
                f"{what} has {v.numel()} entries: expected 1 (global), "
                f"{int(type_index.max()) + 1} (per type) or {n} (per neuron)")

        edge_index = torch.as_tensor(edge_index).long()
        n_edges = edge_index.shape[1]
        W = sd["W"].detach().float().flatten()
        if W.numel() != n_edges:
            raise ValueError(
                f"checkpoint has {W.numel()} edges, edge_index has {n_edges}")
        g = W ** 2                                     # the conductance
        if bool((g < 0).any()):                        # cannot happen; assert anyway
            raise AssertionError("negative conductance after squaring")

        return cls(
            tau_i=_per_neuron(torch.nn.functional.softplus(sd["raw_tau"]), "raw_tau").to(device),
            V_i_rest=_per_neuron(sd["V_rest"], "V_rest").to(device),
            edge_index=edge_index.to(device),
            W=g.to(device),
            E_exc=_per_neuron(sd["E_exc"], "E_exc").to(device),
            E_inh=_per_neuron(sd["E_inh"], "E_inh").to(device),
            edge_is_inh=sd["edge_is_inh"].bool().to(device),
        )

    def reversal_per_edge(self):
        """(E,) the reversal potential each edge drives toward.

        `edge_is_inh` is the PRESYNAPTIC cell's Dale sign; E is indexed by the
        POSTSYNAPTIC cell, so the destination row selects which of the two
        per-neuron reversals applies.
        """
        dst = self.edge_index[1]
        return torch.where(self.edge_is_inh, self.E_inh[dst], self.E_exc[dst])

    def bracket_violation(self, v):
        """How far (E_inh < v < E_exc) is violated by voltages `v` (..., N).

        The squared message cannot represent an edge whose driving force flips
        sign mid-run, so a crossing invalidates the data rather than degrading
        it. Returns (n_exc_crossings, n_inh_crossings, worst_margin); a healthy
        run has 0, 0 and a positive worst margin.
        """
        v = torch.as_tensor(v)
        m_exc = (self.E_exc - v).min()
        m_inh = (v - self.E_inh).min()
        return (int(((self.E_exc - v) <= 0).sum()),
                int(((v - self.E_inh) <= 0).sum()),
                float(torch.minimum(m_exc, m_inh)))


@register_ode_params("drosophila_cx_voltage")
@dataclass
class DrosophilaCxVoltageODEParams(ODEParamsBase):
    """Parameters for the drosophila CX *voltage-rollout* GNN (Branch 0).

    A peer of :class:`FlyVisCurrentODEParams` (both inherit :class:`ODEParamsBase`):
    the CX voltage dataset is generated by rolling the no-TV cv0 RNN teacher
    (``drosophila_cx_pi``) and saving the graded-voltage schema (``tau_i,
    V_i_rest, edge_index, signed W``), so the on-disk fields match FlyVis. It is
    a *separate* model, though — its firing-rate nonlinearity is the teacher's
    ``recurrent_activation`` (default **sigmoid**, ϕ(h)=1/(1+e⁻ʰ); see
    Hidden_Symmetries.pdf p13, N=156), NOT ReLU or softplus. That activation is a
    property of *how the data was generated*, so it is carried in ``ode_params.pt``
    as the ``activation`` field; ``gt_g_phi_func``/``g_phi_label`` dispatch on it.
    The per-neuron gain/bias are not saved with the voltage rollout, so the GT
    g_phi is the shared activation shape; the per-neuron W correction uses the
    base linear-slope fit on the learned curves.

    Registering it here (instead of falling back to FlyVisCurrentODEParams via a
    KeyError) makes the registry symmetric and lets the tester/plotter drop their
    drosophila_cx_voltage special-cases.
    """

    tau_i: torch.Tensor = None  # (N,) time constants
    V_i_rest: torch.Tensor = None  # (N,) resting potentials (~0 for the CX teacher)
    edge_index: torch.Tensor = None  # (2, E)
    W: torch.Tensor = None  # (E,) effective synaptic weights (signed)
    type_names: list = None  # optional cell-type-name list
    activation: str = "sigmoid"  # teacher firing-rate nonlinearity (recurrent_activation)
    ...
    def gt_g_phi_func(self, v):
        """GT firing-rate nonlinearity of the CX teacher, selected by the
        ``activation`` field saved in ode_params.pt (default sigmoid). Per-neuron
        gain/bias are not carried by the voltage rollout, so this is the shared
        curve. Same shape as ``v`` ((n_pts,) or (N, n_pts))."""
        act = (self.activation or "sigmoid").lower()
        if act == "sigmoid":
            z = np.clip(v, -30.0, 30.0)
            return 1.0 / (1.0 + np.exp(-z))
        if act == "tanh":
            return np.tanh(v)
        if act == "softplus":
            beta = 5.0
            return np.where(beta * v > 20, v, np.log1p(np.exp(beta * v)) / beta)
        return np.maximum(v, 0.0)  # relu / fallback

    def g_phi_label(self):
        act = (self.activation or "sigmoid").lower()
        return {
            "sigmoid": r"$\sigma(v_j)=1/(1+e^{-v_j})$",
            "tanh": r"$\tanh(v_j)$",
            "softplus": r"$\mathrm{softplus}(v_j, \beta\!=\!5)$",
            "relu": r"$\mathrm{ReLU}(v_j)$",
        }.get(act, r"$\sigma(v_j)$")
