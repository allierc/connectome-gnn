...
def data_train_gnn(config, erase, best_model, device, log_file=None):
    ...
    if w_init_mode != "randn":
        ...
        _logger.info(
            f"W init mode: {w_init_mode}" + (f" (scale={w_init_scale})" if w_init_mode == "randn_scaled" else "")
        )
    ...
    _logger.info(
        f"learning rates: lr_W {lr_W}, lr {lr}, lr_update {lr_update}, lr_embedding {lr_embedding}, lr_NNR {lr_NNR}"
    )

    optimizer, n_total_params = set_trainable_parameters(
        model=model, lr_embedding=lr_embedding, lr=lr, lr_update=lr_update, lr_W=lr_W, lr_NNR=lr_NNR, lr_NNR_f=lr_NNR_f
    )
    ...
    logger.info(
        f"coeff_W_L1: {tc.coeff_W_L1} coeff_g_phi_diff: {tc.coeff_g_phi_diff} coeff_f_theta_diff: {tc.coeff_f_theta_diff}"
    )
    _logger.info(
        f"coeff_W_L1: {tc.coeff_W_L1} coeff_g_phi_diff: {tc.coeff_g_phi_diff} coeff_f_theta_diff: {tc.coeff_f_theta_diff}"
    )
    ...
    for epoch in range(start_epoch, tc.n_epochs):
        ...
        n_plots_per_epoch = 4
        plot_iterations = (
            set(int(x) for x in np.linspace(Niter // n_plots_per_epoch, Niter - 1, n_plots_per_epoch))
            if n_plots_per_epoch > 0
            else set()
        )
        print(
            f"every {connectivity_plot_frequency} iterations: {Niter} iterations per epoch, plot "
        ...
        for N in pbar:
            ...
            if embedding_frozen and N == unfreeze_at_iteration:
                ...
                lr_embedding = tc.lr_embedding
                optimizer, n_total_params = set_trainable_parameters(
                    model=model, lr_embedding=lr_embedding, lr=lr, lr_update=lr_update, lr_W=lr_W, lr_NNR=lr_NNR
                )
            ...
            if tc.recurrent_training and not tc.neural_ODE_training:
                ...
                if (is_regular_r2 or is_early_r2) and "mlp" not in model_name.lower():
                    ...
                    last_connectivity_r2 = plot_training_flyvis(
                        x_ts,
                        model,
                        config,
                        epoch,
                        ...
                        hidden_ids=_r2_hidden,
                    )
                    last_vrest_r2, last_tau_r2 = compute_dynamics_r2(model, x_ts, config, device, n_neurons)
                    with open(metrics_log_path, "a") as f:
                        f.write(
                            f"{regularizer.iter_count},{last_connectivity_r2:.6f},{last_vrest_r2:.6f},{last_tau_r2:.6f}\n"
                        )
            ...
            if test_neural_field:
                ...
                if (is_regular_r2 or is_early_r2) and not test_neural_field and "_mlp" in model_name:
                    ...
                    and not test_neural_field
                    and ("linear" in model_name or "known_ode" in model_name)
                ):
                ...
                if (has_visual_field) & (N in plot_iterations):
                    ...
                    if last_connectivity_r2 is not None:
                        pbar.set_postfix_str(
                            f"{r2_color(last_connectivity_r2)}R²={last_connectivity_r2:.3f}{ANSI_RESET}"
                        )
                    if tc.save_all_checkpoints:
                        torch.save(
                            {"model_state_dict": model.state_dict(), "optimizer_state_dict": optimizer.state_dict()},
            ...
            if (epoch % tc.umap_cluster_freq == tc.umap_cluster_freq - 1) & (epoch < tc.n_epochs - 1):
                ...
                if umap_results is not None:
                    ...
                    for n_type in range(sim.n_neuron_types):
                        ...
                        plt.scatter(
                            a_umap[pos_np, 0], a_umap[pos_np, 1], s=20, color=type_cmap.color(n_type), edgecolors="none"
                        )
...
def data_train_gnn_RNN(config, erase, best_model, device):
    ...
    # Create model
    model = create_model(model_config.signal_model_name, aggr_type=model_config.aggr_type, config=config, device=device)
