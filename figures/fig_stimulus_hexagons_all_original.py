...
3.3× more voxels than the 217-column flyvis_noise_free variant).

Layout — same two-panel structure as fig_stimulus_hexagons_all.py:
  a) Stimulus series over consecutive 20-ms frames.
  b) 6 × 11 grid — stimulus + 65 cell types, per-hexagon z-score.

Usage
-----
    conda run -n neural-graph-linux python figures/fig_stimulus_hexagons_all_all.py
...
HEX_EDGE_C  = 'black'
HEX_EDGE_W  = 0.12            # thinner outline for the denser lattice
HEX_MARKER_S = 12             # scaled by (217/721) for the 3.3× denser lattice
...
# 3:2 figure aspect (w:h), enlarged so the denser lattice remains legible
fig_w = 30.0
fig_h = 20.0
