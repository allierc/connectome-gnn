...
Output: figures/zebrafish/fig_2_connectome.png
"""
...
def _partition_of(name):
    """Functional partition. IPN-core (943 circuit) joins the recurrent ring
    as its own block; otherwise the six-way proprioception partition."""
...
def _set_partition_tick_labels(ax, part_sorted, *, fontsize=10,
                               rotation=30):
    """Tick labels = the 6 partition group names at each block centre."""
...
def _add_partition_strips(ax, part_sorted, *, alpha_overlay=0.0,
                          ...
                          strips=True):
    """Draw partition colour strips along the top and left of a matrix axes
    plus block-boundary lines, and set xlim/ylim to include the strips.
    With ``strips=False`` only the boundary lines (and any overlay) are
    drawn; the matrix is left flush with the axes. Returns the strip width
    in data units (0 if ``strips=False``).
    """
    ...
    for x in bnd:
        ax.axvline(x, color=boundary_color, lw=boundary_lw, alpha=0.7,
                   zorder=3)
        ax.axhline(x, color=boundary_color, lw=boundary_lw, alpha=0.7,
                   zorder=3)
    boundaries = np.concatenate([[0], bnd + 0.5, [N]])
    strip = max(8, int(N * 0.015)) if strips else 0
    for lo, hi in zip(boundaries[:-1], boundaries[1:]):
        ...
        if alpha_overlay > 0:
            ax.add_patch(Rectangle(
                (lo - 0.5, -0.5), hi - lo, N,
                facecolor=col, edgecolor="none", alpha=alpha_overlay,
                zorder=2,
            ))
    if strips:
...
def _panel_W(ax, W, type_ids, names, ref_nz=None, title="", dilate_iter=1,
    ...
    # Sort by partition (consistent with panel a) when a partition vector
    # is supplied. Reorder rows + cols together so block structure matches.
    ...
    if part_sorted is not None:
        # Partition boundary lines only (no colour strips on the edges).
        # Tick labels = the 6 partition group names from Figure 1
        # (dIPN / IPN12 pool / other / ARTR / pt-IPN1 / motor_efferent)
        # at each block centre — no per-RIPN clutter.
...
def _panel_partition_matrix(ax, W, partition):
    ...
    _add_partition_strips(ax, part_sorted, alpha_overlay=0.18,
                          boundary_color="r", boundary_lw=0.5)
    _set_partition_tick_labels(ax, part_sorted)
    ax.set_xlabel("presynaptic", fontsize=13)
    ax.set_ylabel("postsynaptic", fontsize=13)
...
def main():
    ...
    p.add_argument("--circuit", default=None,
                   help="render a single nominal circuit by registry name "
                        "(e.g. zebrafish_HD_IPN12_943_artr_pt1) as two "
                        "panels: support mask + signed W_con. Bypasses the "
                        "v1/v2 config path.")
    p.add_argument("--device", default="cpu")
    p.add_argument("--output_root", default=None)
    p.add_argument("--out", default=os.path.join(
        here, "fig_2_connectome.png"))
    args = p.parse_args()
    ...
    # --- Single nominal circuit (e.g. 943): support mask + signed W -------
    if args.circuit:
        from connectome_gnn.generators.circuits import get_circuit
        cxo = get_circuit(args.circuit)
        ...
        nnz = int((W != 0).sum())
        print(f"{args.circuit}: N={W.shape[0]} nnz={nnz} "
              f"dens={nnz / W.size:.4f} E={int((W > 0).sum())} "
              f"I={int((W < 0).sum())}")
        for k in PARTITION_ORDER:
            print(f"  {k:18s}  {int((partition == k).sum())}")
        fig, axes = plt.subplots(1, 2, figsize=(12.5, 6),
                                 gridspec_kw={"wspace": 0.34})
        _panel_partition_matrix(axes[0], W, partition)
        _panel_W(axes[1], W, nt, names, ref_nz=None, title="",
                 partition=partition, show_cbar=True)
    ...
    # Tight wspace pulls b and c flush (b's colorbar is suppressed so the
    # only shared scale bar sits to the right of c).
    # Panel a's partition legend now lives BELOW panel a (horizontal,
    # 3 cols × 2 rows), so it can't overflow into panel b. Default-ish
    # wspace is enough to keep the three matrices visually separate.
    fig, axes = plt.subplots(1, 3, figsize=(17, 6),
                             gridspec_kw={"wspace": 0.44})
    _panel_partition_matrix(axes[0], W1, partition)
    _panel_W(axes[1], W1, nt, names, ref_nz=ref_nz, title="",
             partition=partition, show_cbar=False)
    _panel_W(axes[2], W2, nt, names, ref_nz=ref_nz, title="",
             partition=partition, show_cbar=False)

    for k, ax in enumerate(axes.flat):
        ax.text(-0.02, 1.06, "abc"[k], transform=ax.transAxes,
                ha="right", va="bottom", fontsize=14, fontweight="bold")
