"""
Figure: flyvis_noise_05_winner rollout on YouTube-VOS (cv00) — noisy vs noise-free.

Two stacked panels (12 representative cell types, 1 000 frames each):
  a) model rollout vs YouTube-VOS test data with σ = 0.5 process noise
...
Usage
-----
    conda run -n neural-graph-linux python figures/fig_davis_youtube_rollout_noise_05.py

Output
------
    figures/fig_davis_youtube_rollout_noise_05.{png,pdf,jpg}
"""
...
MODEL_NAME  = 'flyvis_noise_05_winner'               # model log dir name
CV00_DS     = 'flyvis_noise_05_winner_cv00'          # noisy-test variant (σ=0.5)
NF_TWIN     = 'flyvis_noise_05_winner_cv00_nf'       # noise-free twin
...
# ---------------------------------------------------------------------------
# Top panel — noisy cv00 (σ=0.5 retained on test via noisy_test_data=True)
# ---------------------------------------------------------------------------
def ensure_cv00_config():
    ...
    _clone_winner(
        CV00_YAML, CV00_DS,
        description=(
            'Noisy cv00 variant of flyvis_noise_05_winner — YouTube-VOS stimulus + '
            'seed 42, noise_model_level=0.5, noisy_test_data=true (so x_list_test '
            'keeps σ=0.5 process noise). Used only as test data for the '
            'noise_05_winner model (figures/fig_davis_youtube_rollout_noise_05.py).'
        ),
...
def ensure_twin_config():
    ...
    _clone_winner(
        NF_YAML, NF_TWIN,
        description=(
            'Noise-free twin of flyvis_noise_05_winner cv00 — same YouTube-VOS videos + '
            'seed 42, noise_model_level=0. Used only as test data for the noise_05_winner '
            'model (see figures/fig_davis_youtube_rollout_noise_05.py).'
        ),
...
def main():
    ...
    # Load — top panel comes from the existing rollout bundle (noisy σ=0.5)
    ...
    # Per-row legend anchored to the right of the residual column (vertical
    # stack so it doesn't steal plot width).
