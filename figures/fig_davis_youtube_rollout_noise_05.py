...
Two stacked panels (12 representative cell types, 1 000 frames each):
  a) model rollout vs DAVIS test data with σ = 0.5 process noise
     (noisy cv00: flyvis_noise_05_cv00)
...
BASE        = 'flyvis_noise_05'                      # condition short-name
MODEL_NAME  = f'{BASE}_yt_per_cond_cv00'             # YT-trained model (fold 0)
CV00_DS     = f'{BASE}_cv00'                         # DAVIS noisy test (σ=0.5)
...
def ensure_cv00_config():
    ...
    _clone_base(
        ...
        description=(
            ...
            'keeps σ=0.5 process noise). Used as test data for the YT-trained '
            f'{MODEL_NAME} model (figures/fig_davis_youtube_rollout_noise_05.py).'
        ),
...
def ensure_twin_config():
    ...
    _clone_base(
        ...
        description=(
            ...
            f'{MODEL_NAME} model (see figures/fig_davis_youtube_rollout_noise_05.py).'
        ),
