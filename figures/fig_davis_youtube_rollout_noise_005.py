"""Figure: YT-trained GNN rollout on DAVIS (cv00) — noisy vs noise-free (σ = 0.05).

Janne-styled per figures/INSTRUCTIONS.md (the previous, larger-font version
is preserved at fig_davis_youtube_rollout_noise_005_original.py):
...
Two stacked panels, 12 representative cell types, 1 000 frames each:
  a) YT-trained GNN rollout vs DAVIS test with σ=0.05 process noise
  b) same model vs noise-free twin of the same DAVIS stimulus

Usage
-----
    conda run -n neural-graph-linux \\
        python figures/fig_davis_youtube_rollout_noise_005.py

Output
------
    figures/fig_davis_youtube_rollout_noise_005.{pdf,png}
"""
...
def ensure_cv00_config():
    ...
    _clone_base(
        ...
        description=(
            ...
            'seed 42, noise_model_level=0.05, noisy_test_data=true. Used as test '
            f'data for YT-trained {MODEL_NAME} '
            '(figures/fig_davis_youtube_rollout_noise_005.py).'
        ),
...
def ensure_cv00_data():
    ...
    # (with noisy_test_data: true) rather than the base flyvis_noise_005.yaml
...
def ensure_twin_config():
    ...
    _clone_base(
        ...
        description=(
            ...
            f'{MODEL_NAME} (figures/fig_davis_youtube_rollout_noise_005.py).'
        ),
...
def main():
    ...
    ax_a, ax_a_r = draw_row(fig, gs, 0, true_top_w, pred_top_w, labels,
                            ...
                            sigma_title=r'low intrinsic noise ($\sigma = 0.05$)')
