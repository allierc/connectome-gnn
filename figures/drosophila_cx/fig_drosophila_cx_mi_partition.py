"""MI-based partition of the recurrent pool, four joint-task models --- the
Drosophila CX companion of ``figures/zebrafish/fig_zebrafish_mi_partition.py``
(zebrafish.tex Fig. 6 / GNN Fig. 17).

Run across four trained joint-task models on the 338-cell CX circuit
(``drosophila_cx_338_v1``): rotation+distance and 2-D path, each cumulative and
leaky. For each recurrent neuron i --- the ring/integrator substrate; the
velocity afferents are excluded by construction (the ``pen_pfn`` gate drives
ω → PEN_a/PEN_b and v_fwd → PFNd/PFNv, so those four types carry the input
signal trivially and are dropped, exactly as RIPN*/pt-IPN* are in the fish) ---
we compute the per-neuron MI on the integrated heading θ and on the forward
displacement d.

The recurrent substrate (236 cells) splits into four anatomical families:

    EPG     : EPG, EPGt                  --- compass / heading bump  (amber)
    ring    : PEG, ER6, Delta7           --- EB ring + PB inhibition (teal)
    PFN     : PFNa                        --- columnar integrator     (rose)
    output  : hDeltaB, PFR_a, PFR_b       --- displacement readout    (purple)

Same layout, axes (common 0--4 bit scale), and dot-size encoding as the fish
figure: the faint per-neuron plug-in MI cloud (n=1, family-coloured), one dot
per cell type, and the whole recurrent pool (black, n≈236), with dot area
growing with group size; light grey diagonals mark constant total information.

This single script produces BOTH the RNN figure (default ``--runs``) and the
GNN figure --- run it with the ``drosophila_cx_gnn_*`` runs and
``--out ...fig_drosophila_cx_mi_partition_gnn.png``.
...
# zebrafish figures dir on path so _despine (open-axes styling) resolves.
sys.path.insert(0, os.path.join(HERE, "..", "zebrafish"))
...
# Recurrent-substrate anatomical families (the bump ring + integrator/readout
# columnar cells). Mirrors the four-family split of the fish figure.
REC_FAMILY_ORDER = ("EPG", "ring", "PFN", "output")
REC_FAMILY_COLOR = {
    "EPG":    "#d29922",   # amber  — compass bump (EPG, EPGt)
    "ring":   "#2aa198",   # teal   — PEG/ER6/Delta7 ring + PB inhibition
    "PFN":    "#c9468a",   # rose   — PFNa columnar integrator
    "output": "#6a51a3",   # purple — hDeltaB/PFR displacement readout
}


def _rec_family(name: str) -> str:
    """Map a recurrent cell-type name to its coarse anatomical family."""
    if name.startswith("EPG"):                                # EPG, EPGt
        return "EPG"
    if name.startswith("PFNa"):                               # columnar integrator
        return "PFN"
    if name.startswith("hDelta") or name.startswith("PFR"):   # displacement readout
        return "output"
    return "ring"                                             # PEG, ER6, Delta7


def _is_recurrent(name: str) -> bool:
    # Afferents are the velocity-gate targets (pen_pfn): ω → PEN_a/PEN_b and
    # v_fwd → PFNd/PFNv. They carry the input signal directly (trivially high
    # MI with ω / v_fwd), so they are excluded from the recurrent code, exactly
    # as RIPN*/pt-IPN* are in the zebrafish figure. PFNa (no d/v suffix) is the
    # recurrent columnar integrator and stays in.
    if name.startswith("PEN"):
        return False
    if name.startswith("PFNd") or name.startswith("PFNv"):
        return False
    return True
...
# decoder lower bound H(y)−CE_test in bits. A DIFFERENT estimator from the
# per-neuron plug-in MIs (comparable across the four columns, but not
# numerically to the individual dots).
...
def _accumulate_hidden(net, u_test, y_test, device, n_trials):
    ...
    scalar ξ. For the 2-D path tasks (4-col target [cosθ,sinθ,x,y]) the
    per-neuron displacement scalar is the RADIAL distance r=|(x,y)| (the proper
    2-D analogue of ξ); ``pos2d`` returns (x,y) so callers can score the JOINT
    2-D position at the population level."""
    hs, thetas, ds, xs, ys = [], [], [], [], []
...
# The four joint-target models compared across the columns. Each carries both a
# heading target θ and a forward-displacement target (scalar ξ for the "both"
# task, the 2-D path for "position_2d"), so the I(h;θ) vs I(h;d) partition is
...
def _compute_run(run_basename: str, data_root: str, n_trials: int, device,
    ...
    # Fixed, SYMMETRIC thresholds (shared across all four models so the columns
    # are directly comparable) rather than per-model medians.
    ...
    # one dot per cell type, one per family, and one for the whole pool.
    # Cross-validated decoder lower bound; for the 2-D models the displacement
    # target is the joint (x,y) position. Dot size ∝ count, so single-cell-type
    # dots (small) sit low and the whole pool (large) sits near the ceiling.
    Hrec = H[:, rec_ix]
    Tn = Hrec.shape[0]
...
def main():
    ...
    p.add_argument("--out", default=os.path.join(
        HERE, "fig_drosophila_cx_mi_partition.png"))
    ...
    # spreading cell type / whole pool clearly apart).
    S_A, S_P = 14.0, 0.55
    ...
    for j, r in enumerate(results):
        ...
        if j == len(results) - 1:                 # dot-size key on panel d
            size_ref = [(1, "single cell"), (20, "cell type"), (236, "all rec")]
