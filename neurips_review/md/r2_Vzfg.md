...
|---|---|---|---|---|---|---|---|---|
| random assemblies [Allier 2026] | strong | chaotic | $1.00$ | — | — | — | — | — |
...
*Cells are $R^2_{\widehat W}$ at process noise $\sigma$; ODE = Known-ODE oracle. * provisional, single seed, still training; n.c. = did not converge; — = not reported.*
...
*On [1].* Pruned Dale-constrained RNNs are a good generator here. We do not assume Dale's law — $\widehat W_{ij}$ is unconstrained, $g_\phi^2$ carries only magnitude — so such a benchmark would test sign recovery, not sign compliance.
...
Two entries carry less than they appear to. The off-graph fraction of $\sum_{i \neq j}|J_{ij}|$ is zero by construction for both our models, so it reports the hypothesis class, not what was learned; and sign agreement of $0.88$ reduces, averaged over frames, to sign recovery of $\mathbf{W}$, the oracle sitting at the same $0.882$.
