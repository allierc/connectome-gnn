...
structure that makes the biological inverse problem hard. It is
...
through the known kernel to give an estimate of voltage, which we score against
the true voltage. That estimate reaches $r = 0.90$ under (i), $0.35$ under (iii)
and $0.23$ under (iv). Saturation is instructive: $r$ barely moves
($0.90 \to 0.88$) while RMSE triples ($0.12 \to 0.37$), since a linear inverse
cannot undo a static nonlinearity. Amplitudes carrying the weight information
stay compressed while the correlation looks healthy. Recovery arms running.
\textbf{[pending]}
