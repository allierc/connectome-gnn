...
recording while those pushing it off are undone by the attractor. Fit quality
stays high while they stay unknown.
...
We have now built it, as four observables on the same trajectories:
(i)~the GCaMP6f kernel alone; (ii)~kernel plus indicator saturation, the
response flattening once the excursion passes $K_d$, set at $1.57\times$ the
trace SD; (iii)~kernel plus a $500$~ms exposure ($2$~Hz imaging); (iv)~all of
these with shot noise matched in SNR to our $\gamma$ rows. Before any fitting,
the $\dot C$ signal that identifies $\Mat{W}$ retains, against (i), $0.40$ under
(ii), $0.70$ under (iii) and $0.26$ under (iv). Deconvolving the kernel to
estimate voltage from the calcium trace, as an experimenter would, and scoring
that estimate against the true voltage gives $r = 0.90$ under (i), $0.35$ under
(iii) and $0.23$ under (iv). Saturation is the instructive case: $r$ barely moves
($0.90 \to 0.88$) while RMSE triples ($0.12 \to 0.37$), since a linear inverse
cannot undo a static nonlinearity. The amplitudes carrying the weight
information stay compressed while the correlation looks healthy. Recovery arms
are running. \textbf{[pending]}
...
scale at which this test discriminates.
...
ground-truth Jacobian and restricting to those moves $R^2$ by $<10^{-3}$.
