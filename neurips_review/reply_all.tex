...
it does recover a known adjacency ($F_1 = 0.88$--$0.93$, one-step forecasting),
but the units are electrode channels rather than identified neurons, and the real
recordings carry no ground-truth connectivity: validation there is by recovering
channel spatial proximity. Its own finding that recovery tracks the training
...
scale first --- the same gain degeneracy our $\mu_2$ anchor addresses (W5). Ours
is $4\times10^5$ independently drawn weights with no such symmetry. The failure
...
weights on it: a necessary condition, and we will present it as one.
