...
recording while those pushing it off are undone by the attractor and constrained
by nothing. Fit quality stays high while they stay unknown.
...
activity. That is our mechanism, so we will cite our process-noise result as a
measured instance of their prescription rather than an independent finding. The
failures differ: theirs is a false edge where no synapse exists, ours a true edge
unidentifiable against its same-type neighbours.

\emph{On [1].} Pruned Dale-constrained RNNs are a good generator here. We do
not assume Dale's law: $\widehat W_{ij}$ is unconstrained and $g_\phi^2$ carries
only magnitude. Such a benchmark would test sign recovery, not sign compliance.
...
subsampling alone, measurement noise ($0.63$ at $\gamma = 0.1$) an additive term
alone.

We have now built the composition: a GCaMP6f kernel, Hill saturation at
$K_d = 1.57\times$ the trace SD, a $500$~ms exposure ($2$~Hz imaging) and shot
noise matched in SNR to our $\gamma$ rows, on the same trajectories. Before any
fitting, the derivative signal identifying $\Mat{W}$ falls to $0.40$ of baseline
under saturation, $0.70$ under rate and $0.26$ under all three. Deconvolving the
indicator kernel, as an experimenter would, recovers voltage at $r = 0.90$ from
blur alone, $0.35$ once the rate drops and $0.23$ for the full model. Saturation
is the instructive case: $r$ barely moves ($0.90 \to 0.88$) while RMSE triples
($0.12 \to 0.37$). A linear inverse cannot undo a static nonlinearity, so the
amplitudes carrying the weight information stay compressed while the correlation
looks healthy. Recovery arms are running. \textbf{[pending]}
...
Current injection does take it off the manifold, and a gap opens that grows with
...
scale at which this test discriminates. The oracle stays above $0.998$, exact by
construction; the GNN degrades gently.
...
model and an oracle that knows the equation, not between a mechanistic model and
one that only predicts. The recurrent-MLP and EED checkpoints are held by a
co-author who is away, so that column stays blank and will be filled during the
discussion; it needs rollouts, not retraining.
...
see the same states, on the noise-free blank50 run Supp.~Fig.~18 uses.
...
reduces, averaged over frames, to sign recovery of $\Mat{W}$, the oracle at the
same $0.882$.
...
in the order of the letter, W1 to W5 then Q1 to Q2, and where a point recurs across
