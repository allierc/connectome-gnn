...
class EyeG(nn.Module):
    ...
    a buffer, so `.parameters()` is empty and the optimiser cannot retune the body.
        ...
        x_inf = g(m)                            static, measured (steady state, m = motor neuron output (6x1))
        xdd + C xd + K x = K x_inf              linear, fitted (fixed based on soft body model calibration, x = gaze angles (3x1))
    """
...
class CTRNNEyeG(nn.Module):
    ...
    def __init__(self, eye, hidden=64, tau0=0.5, dt=1 / 60):
        ...
        # learnables
        self.log_tau = nn.Parameter(torch.full((hidden,), float(np.log(tau0)))) # 1x64
        self.W = nn.Parameter(torch.zeros(hidden, hidden))
        self.enc = nn.Linear(2, hidden)                 # (xdot, ydot), W^in
        self.mot = nn.Linear(hidden, eye.n_act)         # decoder, W^out
        with torch.no_grad():
            self.enc.weight.mul_(0.5)

    def forward(self, u, want_states=False):
        alpha = (self.dt / self.log_tau.exp()).clamp(1e-4, 1.0) # (delta t)/tau
        B, T, _ = u.shape #(B?)
        #h = voltage (v)
        h = torch.zeros(B, self.W.shape[0], device=u.device, dtype=u.dtype)
        drive = self.enc(u)# u is the velocities that go into the encoder (2xT)
        rates = []
        for t in range(T):
            r = torch.tanh(h)
            rates.append(r)
            h = h + alpha * (-h + r @ self.W.T + drive[:, t])
        # firing rates based on voltages
