...
At clean noise with GT edges, known_ode optimization should achieve near-perfect parameter recovery.
...
1. **Full Log (append-only)**: `drosophila_cx_known_ode_gt_edges_Claude_analysis.md`
   - Append every iteration's log entry (4 entries per batch)
   - Never read — human record only

2. **Working Memory (read + update every batch)**: `drosophila_cx_known_ode_gt_edges_Claude_memory.md`
...
Example: "lr_W=1e-3 with GT edges achieves connectivity_R2 > 0.7 robustly (3/3 iterations, all seeds > 0.65, CV < 3%)"
...
Example: "Does GT edges allow higher learning rates than FC? Only iter 1 tested."
...
Example: "Hypothesis: 'GT edges eliminates sparsity problem, improving recovery to > 0.9' — Falsified by iter 1 (best only 0.72). Revised: 'GT edges helps but parameter estimation still challenging; requires careful lr tuning.'"
