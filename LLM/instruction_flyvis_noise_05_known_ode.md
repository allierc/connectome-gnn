...
**Primary objective**: Establish a **robust config** that achieves **connectivity_R2 > 0.7 across all 4 seeds with CV < 5%**, demonstrating how known_ode handles high-noise regimes where optimization difficulty increases dramatically.

**Why this matters**: known_ode is an inverse problem — we have the ODE dynamics `f(v, W)` known, and we learn connectivity W and biophysical parameters (tau_i, V_rest_i) directly from noisy neural observations. High-noise data (noise_model_level=0.5) presents a severe challenge; establishing how much performance loss is due to noise vs optimization difficulty helps us understand the limits of parameter recovery under realistic conditions.
...
In high-noise regimes (noise_model_level=0.5), the gradient signal is severely corrupted, and regularization becomes critical to prevent overfitting to noise. This baseline establishes the performance ceiling for high-noise recovery.
...
- Dataset: `fly/flyvis_noise_05` (DAVIS visual input, 64,000 frames)
- Noise model: `noise_model_level=0.5, measurement_noise_level=0.0`
...
**Trade-off**: Regularization is critical on noisy data. Balance: enough to stabilize, not so much that it prevents convergence to true W.
...
**Robustness criteria** (high-noise data):
...
**Hypothesis**: "Higher learning rates accelerate convergence on noisy data without divergence; optimal LR will achieve connectivity_R2 > 0.65 within 1 epoch"
...
**Expected outcome**: Identify range where convergence is fastest and stable across seeds.

### Block 2 (iter 13-24): W Regularization
**Hypothesis**: "On noisy data, regularization prevents overfitting to noise. Stronger regularization improves connectivity_R2."
...
**Expected outcome**: Validate that noisy data requires regularization.

### Block 3 (iter 25-36): W Initialization + LR
**Hypothesis**: "Initialization mode interacts with LR. Better initialization accelerates convergence on noisy data."
...
**Expected outcome**: Determine best initialization for noisy data.

### Block 4 (iter 37-48): Batch Size + LR
**Hypothesis**: "Larger batch sizes reduce gradient noise variance. Interact with LR to stabilize convergence on noisy data."
...
**Expected outcome**: Quantify batch size effect on convergence stability.
...
**Final test**: Run the best config from Blocks 1-6 on 4 independent seeds to validate connectivity_R2 > 0.7 with CV < 5%.

## File Structure

You maintain THREE files:

1. **Full Log (append-only)**: `flyvis_noise_05_known_ode_Claude_analysis.md`
   - Append every iteration's log entry (4 entries per batch)
   - Never read — human record only

2. **Working Memory (read + update every batch)**: `flyvis_noise_05_known_ode_Claude_memory.md`
...
Example: "coeff_W_L2=0.00015 is necessary for robust convergence on noise_05 (3/3 iterations, all seeds > 0.70, CV < 3%)"
...
Example: "Does batch_size=8 improve convergence stability? Only iter 1 tested; seeds 1-2 > 0.72 but seeds 3-4 < 0.68."
...
Example: "Hypothesis: 'Reduced regularization improves connectivity_R2 on noisy data' — Falsified by iter 8 (coeff_W_L2=0 → mean R2=0.61, CV=12%). Revised: 'On noise_05, baseline regularization is optimal; weaker regularization causes seed-dependent overfitting.'"
...
- **Stable-Robust**: all 4 slots connectivity_R2 > 0.7 AND CV < 3% — TARGET
- **Robust**: all 4 slots connectivity_R2 > 0.7, CV 3-5%
- **Partially robust**: 2-3 slots connectivity_R2 > 0.7
- **Fragile**: 0-1 slots connectivity_R2 > 0.7
- **DISQUALIFIED**: any slot < 0.65 — reject config immediately
...
2. Copy config from `log/Claude_exploration/LLM_flyvis_noise_05_known_ode/config/iter_XXX_slot_YY.yaml`
3. Save to `config/fly/flyvis_noise_05_known_ode_winner.yaml` with header:

```yaml
# Winner config: flyvis_noise_05_known_ode_winner.yaml
...
- Read base config — note `noise_model_level=0.5` (high noise)
- Initialize 4 slot configs for Block 1 (learning rate sweep)
- **Initial hypothesis**: "On noisy data, conservative learning rates prevent divergence; optimal LR will achieve connectivity_R2 > 0.65 while maintaining CV < 5%"
