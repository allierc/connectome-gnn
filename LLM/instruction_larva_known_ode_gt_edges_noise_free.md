# Drosophila Larva — Known ODE Exploration (GT Edges, Noise Free)

## Goal

Maximize **connectivity_R2** (PRIMARY) for the **Drosophila larva two-population motor model** using the **known_ode model** with **GT edge topology** (4,222 edges) and **clean dynamics** (noise-free).

The known_ode model uses the **exact activation function**: `g_phi = softplus(v + b)` and **exact dynamics**: `dv/dt = (-v + g * msg + I)`. All parameters are **learned from data**: W, g (gain per neuron), bias per neuron. This combines perfect structural knowledge with known topology and clean data — the best-case scenario.

**Starting hypothesis**: "Known ODE + GT edges + noise-free achieves near-perfect W R2 (>0.9)"
...
At this clean noise level with GT edges and perfect structure, known_ode should achieve excellent parameter recovery.
...
- 2,400 frames, delta_t=0.05, **noise_model_level=0.0**
...
- **GT edges + clean data is the best-case scenario**: Known topology removes sparsity problem, clean data provides clear signal. This should approach the theoretical ceiling.
- **Near-perfect W recovery expected**: Even GNN achieves high R2 with GT edges + noise-free. Known ODE should match or exceed.
...
Destination: `config/larva/larva_known_ode_gt_edges_noise_free_winner.yaml`

```yaml
# Winner config: larva_known_ode_gt_edges_noise_free_winner.yaml
...
1. **Full Log (append-only)**: `larva_known_ode_gt_edges_noise_free_Claude_analysis.md`
2. **Working Memory**: `larva_known_ode_gt_edges_noise_free_Claude_memory.md`
...
- Hypothesis: "Known ODE + GT edges + noise-free achieves connectivity_R2 > 0.9 with default parameters"
...
```markdown
# Working Memory: larva_known_ode_gt_edges_noise_free
