# FlyVis MLP Baseline — Rollout Optimization (noise=0.05)

## Goal

Optimize the **MLP baseline** hyperparameters for lowest possible autoregressive rollout error ~
O(1e-2) or lower on the flyvis model with **moderate noise** (sigma=0.05). Two sub-goals:
...
Applying that same noise-free config to noise=0.05 data yields RMSE=0.1386, Pearson r=0.776 — a
significant degradation. **Noise is added only to the training data; test/validation rollouts are
noise-free.** The goal is to match or beat the noise-free RMSE (0.0799) despite training on noisy
data.

## Scientific Context

The core research question is: **Can a flat, graph-free MLP function approximator achieve stable
autoregressive rollout on the simulated FlyVis model when trained on moderately noisy data?**

At sigma=0.05, the noise magnitude is small relative to the signal but corrupts the training
derivatives (dv/dt targets). Key challenges:

- The model must learn the underlying clean dynamics from noisy training data
- Per-step noise-induced errors may compound differently in autoregressive rollout
- The optimal hyperparameters may differ from noise-free — this must be determined experimentally
