...
Test **known_ode performance with moderate-low noise** for the **Drosophila visual system** with noise level 0.005 (DAVIS input). The goal is to find a **config** that achieves **connectivity_R2 > 0.85 on ALL 4 seeds with CV < 3%**, demonstrating good performance when noise is mild.

**Primary objective**: Establish a **robust config** that achieves **connectivity_R2 > 0.85 across all 4 seeds with CV < 3%**, bridging the gap between clean data (noise_model_level=0.0) and high-noise regimes (noise_model_level=0.5).

**Why this matters**: known_ode is an inverse problem — we have the ODE dynamics `f(v, W)` known, and we learn connectivity W and biophysical parameters (tau_i, V_rest_i) directly from noisy neural observations. Moderate-low noise provides a realistic test case where the signal is recoverable but noise still presents challenges. Understanding performance at this noise level helps establish the practical operating range.
...
At moderate-low noise (noise_model_level=0.005), optimization should be more tractable than high-noise regimes, but regularization still matters. This regime tests whether the learning rates and regularization found in noisier regimes generalize, or if cleaner data requires different hyperparameters.
...
- Dataset: `fly/flyvis_noise_005` (DAVIS visual input, 64,000 frames)
...
**Trade-off**: Moderate-low noise allows slightly reduced regularization compared to high-noise regimes.
...
**Robustness criteria** (moderate-low noise data):
...
**Hypothesis**: "Moderate noise allows faster learning than high-noise regime. Optimal LR will achieve connectivity_R2 > 0.80 within 1 epoch"
...
**Expected outcome**: Identify LR range for optimal convergence at this noise level.

### Block 2 (iter 13-24): W Regularization
**Hypothesis**: "Moderate noise may tolerate reduced regularization. Test whether lighter regularization improves connectivity_R2."
...
**Expected outcome**: Validate regularization trade-off at moderate-low noise.

### Block 3 (iter 25-36): W Initialization + LR
**Hypothesis**: "Initialization mode may matter less at moderate noise. Test whether initialization differences persist."
...
**Expected outcome**: Determine whether initialization effects carry over from noisier regimes.

### Block 4 (iter 37-48): Batch Size + LR
**Hypothesis**: "Moderate noise reduces gradient noise variance naturally. Test whether larger batches still help."
...
**Expected outcome**: Quantify batch size effect at moderate-low noise.
...
**Final test**: Run the best config from Blocks 1-6 on 4 independent seeds to validate connectivity_R2 > 0.85 with CV < 3%.

## File Structure

You maintain THREE files:

1. **Full Log (append-only)**: `flyvis_noise_005_known_ode_Claude_analysis.md`
   - Append every iteration's log entry (4 entries per batch)
   - Never read — human record only

2. **Working Memory (read + update every batch)**: `flyvis_noise_005_known_ode_Claude_memory.md`
...
Example: "lr_W=0.0009 achieves connectivity_R2 > 0.85 robustly on noise_005 (3/3 iterations, all seeds > 0.84, CV < 2%)"
...
Example: "Does w_init_mode=randn_scaled improve stability? Only iter 2 tested with mixed results."
...
Example: "Hypothesis: 'Aggressive LR accelerates convergence on moderate noise' — Falsified by iter 3 (lr_W=0.002 caused CV=8%, only 2/4 seeds > 0.84). Revised: 'Moderate noise requires balanced LR; lr_W > 0.001 increases seed variance.'"
...
2. Copy config from `log/Claude_exploration/LLM_flyvis_noise_005_known_ode/config/iter_XXX_slot_YY.yaml`
3. Save to `config/fly/flyvis_noise_005_known_ode_winner.yaml` with header:

```yaml
# Winner config: flyvis_noise_005_known_ode_winner.yaml
...
- Read base config — note `noise_model_level=0.005` (moderate-low noise)
- Initialize 4 slot configs for Block 1 (learning rate sweep)
- **Initial hypothesis**: "Moderate-low noise allows faster learning than high-noise regime. Optimal LR will achieve connectivity_R2 > 0.80 while maintaining CV < 3%"
...
```markdown
# Working Memory: flyvis_noise_005_known_ode

## Paper Summary (update at every block boundary)

- **known_ode on moderate-low noise**: [What makes known_ode perform at noise_model_level=0.005, which parameters matter most, best connectivity_R2 achieved, comparison to high-noise regime]
- **Robustness findings**: [How stable is the best config across seeds? Key insights about regularization trade-offs at moderate noise.]
