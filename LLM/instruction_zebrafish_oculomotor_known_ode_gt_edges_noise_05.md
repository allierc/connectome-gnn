# Zebrafish Oculomotor — Known ODE Exploration (GT Edges, Noise 0.5)

## Goal

Maximize **connectivity_R2** (PRIMARY) for the **zebrafish oculomotor integrator** using the **known_ode model** with **GT edge topology** (3,213 edges) and **strong intrinsic noise (sigma=0.5)**.

The known_ode model uses the **exact activation function**: `g_phi = identity (linear)` and **exact dynamics**: `dv/dt = (-v + W @ v + I)`. All parameters are **learned from data**: W. This combines perfect structural knowledge with known topology and strong noise.

**Starting hypothesis**: "Known ODE + GT edges + noise=0.5 achieves good W R2 (>0.5), despite strong noise, due to GT constraint"
...
At strong noise with GT edges, recovery is challenging. GT edges provide strong constraint but noise adds fundamental limits to identifiability.
...
- **609 neurons**, 6 cell types, **3,213 GT edges**
...
| `noise_model_level`       | 0.5     | **FIXED** — strong intrinsic noise level               |
...
| ----- | ------------------------------ | ------------------------------------------- | -------------------------------------------------------------------------- |
| 1     | **lr_W + lr sweep**            | `lr_W`, `lr`                                | lr_W: {5e-5, 1e-4, 5e-4, 1e-3}, lr: {1e-4, 5e-4, 1e-3, 5e-3}            |
| 2     | **Training volume**            | `data_augmentation_loop`, `n_epochs`        | DAL: {100, 200, 500, 1000}, n_epochs: {2, 4, 8}. Strong noise needs extensive training. |
| 3     | **W regularization**           | `coeff_W_L1`, `coeff_W_L2`, `coeff_W_sign` | W_L1: {0, 1e-6, 1e-5}, W_L2: {1e-6, 1e-5, 1e-4}, W_sign: {0, 0.1, 0.5}  |
| 4     | **Batch size**                 | `batch_size`                                | batch_size: {2, 4, 8, 16}. Larger batches may help smooth strong noise.   |
| 5-8   | **Free exploration**           | Any parameter                               | Consolidate, ceiling-breaking, final robustness                            |

### Context

- **GT edges + strong noise**: Topology provides strong constraint but strong noise fundamentally limits identifiability. This tests the limit of topology vs noise tradeoff.
- **Strong regularization likely needed**: W_L2 and W_sign become critical stabilizers under overwhelming noise.
...
Destination: `config/zebrafish_oculomotor/zebrafish_oculomotor_known_ode_gt_edges_noise_05_winner.yaml`

## File Structure

1. **Full Log (append-only)**: `zebrafish_oculomotor_known_ode_gt_edges_noise_05_Claude_analysis.md`
2. **Working Memory**: `zebrafish_oculomotor_known_ode_gt_edges_noise_05_Claude_memory.md`
...
- Hypothesis: "Known ODE + GT edges + noise=0.5 maintains good connectivity_R2 (>0.5) robustness despite strong noise"
...
```markdown
# Working Memory: zebrafish_oculomotor_known_ode_gt_edges_noise_05

## Paper Summary (update at every block boundary)

- **Known ODE + GT edges + strong noise**: [pending]
