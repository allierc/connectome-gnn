...
- **CX FC**: noise-free=0.804 -> noise05=0.999 (near-perfect)
- **Zebrafish FC**: noise-free=0.022 -> noise05=0.988 (45x improvement)
- **Larva GT**: noise-free=0.908 -> noise05 exploration achieved strong results
...
3. Save it to `config/larva/larva_fc_noise05_winner.yaml` with a YAML comment header:

```yaml
# Winner config: larva_fc_noise05_winner.yaml
...
Destination: `config/larva/larva_fc_noise05_winner.yaml`
...
1. **Full Log (append-only)**: `larva_fc_noise05_Claude_analysis.md`
   - Append every iteration's log entry (4 entries per batch)
   - Never read — human record only

2. **Working Memory (read + update every batch)**: `larva_fc_noise05_Claude_memory.md`
...
```markdown
# Working Memory: larva_fc_noise05
