...
**Keys that no longer exist.** `Wij_R2`, `Wij_R2_all`, `Eij_n` and
`w_scale` were renamed or removed when parameter extraction was unified. In the ANALYSIS
LOG the gate is `Eij_gate`; `fit_r2_median`, `w_r2_scaled` and `w_scale` survive only as
column names inside `tmp_training/gnn_conductance_fit.log`, so use those spellings when
you quote that file and the key spellings when you quote the log.
...
`tmp_training/Eij.log`, so an entry from before this change that omits them is
not a failed slot. `Eij_*` appears only on conductance-generated data; `msg_i_*`
...
| --- | --- | --- | --- | --- |
| 1 | Robustness | Baseline variance | none — all 4 slots identical | Establish CV of `Wij_R2`, `tau_R2`, `V_rest_R2`, and the trajectory of `Eij_gate` |
