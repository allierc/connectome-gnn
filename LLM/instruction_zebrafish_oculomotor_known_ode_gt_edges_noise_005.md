# Zebrafish Oculomotor — Known ODE Exploration (GT Edges, Noise 0.05)

## Goal

Maximize **connectivity_R2** (PRIMARY) for the **zebrafish oculomotor integrator** using the **known_ode model** with **GT edge topology** (3,213 edges) and **intrinsic noise (sigma=0.05)**.

The known_ode model uses the **exact activation function**: `g_phi = identity (linear)` and **exact dynamics**: `dv/dt = (-v + W @ v + I)`. All parameters are **learned from data**: W. This combines perfect structural knowledge with known topology and mild noise.

**Starting hypothesis**: "Known ODE + GT edges + noise=0.05 achieves near-perfect W R2 (>0.9)"

GT edges + noise combination: topology constrains the space, noise breaks linear degeneracy.
...
At mild noise with GT edges and perfect linear structure, known_ode should achieve excellent parameter recovery.
...
- **Synergy of topology + noise**: GT edges constrain the space, noise breaks linear degeneracy. This should achieve near-perfect recovery.
- **Second-best scenario**: GT edges + noise is superior to GT edges alone (which faces linear degeneracy).
...
Destination: `config/zebrafish_oculomotor/zebrafish_oculomotor_known_ode_gt_edges_noise_005_winner.yaml`

## File Structure

1. **Full Log (append-only)**: `zebrafish_oculomotor_known_ode_gt_edges_noise_005_Claude_analysis.md`
2. **Working Memory**: `zebrafish_oculomotor_known_ode_gt_edges_noise_005_Claude_memory.md`
...
- Hypothesis: "Known ODE + GT edges + noise=0.05 achieves connectivity_R2 > 0.9 robustly"
...
```markdown
# Working Memory: zebrafish_oculomotor_known_ode_gt_edges_noise_005
