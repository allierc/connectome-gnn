# Drosophila CX (Noise 0.5) — LLM Exploration

## Goal

Maximize **connectivity_R2** for the **Drosophila central complex ring attractor** (Beiran & Litwin-Kumar 2023, Figure 5) under **intrinsic noise (sigma=0.5)**.

This exploration starts from the **best noise=0.05 config** and tests whether stronger noise further improves or degrades connectivity recovery. The noise dose-response is already established for flyvis (noise-free=0.926, noise_005=0.985, noise_05=0.990 — monotonically improving) and being tested for zebrafish (noise_005=0.918). For CX, noise=0.05 achieved 0.972 best (0.845 mean over 6 seeds) — a major improvement over clean FC (0.804 best, 0.574 mean). The question is whether noise=0.5 pushes even higher or starts to overwhelm the ring attractor dynamics.

Data is **re-generated each iteration** with a different seed to verify seed independence.

### Parent config (best noise=0.05, from 76-iter exploration)
...
| `hidden_dim`              | 96      | MLP hidden dimension (from noise_005 winner)            |
| `embedding_dim`           | 4       | Embedding dimension (from noise_005 winner)             |
...
| `coeff_W_L2`              | 5e-5    | L2 penalty on W (from noise_005 winner)                 |
| `coeff_W_sign`            | 0       | Dale's law penalty                                     |
| `use_gt_edges`            | false   | Fully connected graph (N^2-N = 22,952 edges)           |
| `dale_law`                | true    | **Enforce Dale's law** (from noise_005 winner — critical stabilizer) |
...
| ----- | ------------------------------ | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| 1     | **Baseline validation**        | None (robustness test)                                                     | Run best noise_005 config with noise=0.5 across 4 seeds. Does 10x more noise help or hurt?                       |
| 2     | **Regularization re-tune**     | `coeff_W_L1`, `coeff_W_L2`, `coeff_W_sign`                                | W_L1: {1e-6, 3e-6, 1e-5, 5e-5}, W_L2: {1e-5, 5e-5, 1e-4}, W_sign: {0, 0.01, 0.05}. Strong noise may need stronger or weaker regularization. |
| 3     | **Training volume re-tune**    | `data_augmentation_loop`, `n_epochs`, `batch_size`                         | DAL: {70, 105, 200}, n_epochs: {2, 4}, batch_size: {2, 4}. Higher noise = noisier gradients — more averaging (DAL/epochs/bs) may help. |
| 4     | **Architecture**               | `hidden_dim`, `embedding_dim`                                              | hidden_dim: {64, 96, 128}, embedding_dim: {2, 4}. Stronger noise may need more capacity.                        |
...
- **Noise dose-response is the key question**: noise=0.05 already improved CX from 0.574→0.845 mean. Does 10x more noise (0.5) continue the trend (as in flyvis) or start to degrade?
- **dale_law=true is established**: The noise_005 exploration proved this is the single biggest lever. Keep it unless evidence clearly says otherwise.
- **f_theta_msg_diff=50 is established**: From noise_005. May need adjustment at higher noise.
- **Signal-to-noise ratio drops dramatically**: At sigma=0.5, noise is comparable to signal amplitude. The GNN must extract connectivity from much noisier gradients. Batch averaging (higher DAL or bs) may be critical.
- **Spectral radius monitoring**: At noise=0.05, best configs achieved spectral_radius=2.449 (96.2% of true 2.545). Monitor whether noise=0.5 disrupts this.
...
3. Save it to `config/drosophila_cx/drosophila_cx_noise_05_winner.yaml` with a YAML comment header:

```yaml
# Winner config: drosophila_cx_noise_05_winner.yaml
...
Destination: `config/drosophila_cx/drosophila_cx_noise_05_winner.yaml`
...
1. **Full Log (append-only)**: `drosophila_cx_noise_05_Claude_analysis.md`
   - Append every iteration's log entry (4 entries per batch)
   - Never read — human record only

2. **Working Memory (read + update every batch)**: `drosophila_cx_noise_05_Claude_memory.md`
...
- Read base config — the parent noise_005 config + noise_model_level=0.5 IS the baseline.
- Block 1 is a **robustness test**: all 4 slots use the same config (different seeds).
- Hypothesis: "The best noise=0.05 config with noise=0.5 maintains connectivity_R2 >= noise=0.05 baseline (0.972 best, 0.845 mean) — stronger noise continues to improve recovery as seen in flyvis"
...
```markdown
# Working Memory: drosophila_cx_noise_05
