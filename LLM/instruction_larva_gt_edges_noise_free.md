...
| 4     | **Regularization + Dale's law** | `coeff_W_L2`, `coeff_W_sign`, `dale_law`, `coeff_g_phi_diff`, `coeff_f_theta_diff`, `coeff_f_theta_msg_diff` | W_L2: {5e-6, 1e-5, 2e-5}, W_sign: {0, 0.05, 0.2}, dale_law: {false, true}, g_phi_diff: {500, 1000, 1500}, f_theta_diff: {0, 10, 50} (leak), f_theta_msg_diff: {0, 10, 50}. Monitor dale_law_score in all iterations. |
...
3. Save it to `config/larva/larva_gt_edges_noise_free_winner.yaml` with a YAML comment header:

```yaml
# Winner config: larva_gt_edges_noise_free_winner.yaml
...
Destination: `config/larva/larva_gt_edges_noise_free_winner.yaml`
