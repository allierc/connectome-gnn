# FlyVis Null Edges (100%) — LLM Exploration

## Goal

Maximize **connectivity_R2** for the FlyVis connectome model with **100% extra null edges** (868,224 total edges: 434,112 real + 434,112 null per-column). The GNN must learn W ≈ 0 for null edges and correct W for real edges.

The March 2026 runs (`_ne_00`–`_ne_03`) achieved **conn_R2 ≈ 0.98, tau_R2 ≈ 0.99** with all MLP/W penalties = 0. The goal is to reproduce this and determine whether any regularization can be safely added.
...
python GNN_LLM.py -o generate_train_test_plot_Claude flyvis_noise_005_null_edges_pc_100 iterations=48 --cluster --resume
```

The pipeline auto-creates `config/fly/flyvis_noise_005_null_edges_pc_100_Claude_00.yaml` through `_03.yaml` on first run. **Do not create these files manually.**
...
- **868,224 total edges**: 434,112 real (W ≠ 0) + 434,112 null per-column (W = 0)
...
**coeff_g_phi_weight_L1 > 0** with n_epochs=1 and regul_annealing_rate=0: the L1 gradient dominates the connectivity gradient (868K edges → small gradient per edge), driving W→0 → conn_R2≈0.002. Do NOT set g_phi_weight_L1 > 0 in n_epochs=1 mode.
...
**~100 min per slot on A100** (868K edges). Data generation: ~15 min/slot. Total budget: ~120 min/slot.
...
|-------|-------------|----------------------------------------|-------------------------------------------------------------------|-------------------------------------------------|
| 1     | Robustness  | **Baseline validation**                | None (all penalties=0, n_epochs=1)                                | Verify March 2026 working condition (conn≈0.98) |
...
- **Block 1**: Confirm conn_R2 ≈ 0.98 across all 4 null-edge placements. If fragile (variance > 10%), investigate before proceeding.
- **Block 2**: The baseline LRs may not be optimal — explore systematically. Best LR config becomes the new parent.
...
3. Save to `config/fly/flyvis_noise_005_null_edges_pc_100_winner.yaml` with header:

```yaml
# Winner config: flyvis_noise_005_null_edges_pc_100_winner.yaml
...
Examples:
- ✓ "W_L1 with annealing (n_epochs=2) safely enables null-edge sparsification without collapsing connectivity (3/3 iterations, CV < 5%)"
- ✓ "Direct MLP weight penalties (n_epochs=1, rate=0) drive conn_R2→0 with 868K edges — incompatible with null edge structure"
...
- Theoretical predictions about null-edge recovery
...
- Hypothesis: "The March 2026 working condition (conn_R2 ≈ 0.98) is reproducible with the current template across 4 different null-edge placements."
...
```markdown
# Working Memory: flyvis_noise_005_null_edges_pc_100
...
- **coeff_g_phi_weight_L1 > 0.1 (n_epochs=1, rate=0)**: conn_R2≈0.002 — L1 gradient dominates connectivity gradient with 868K edges.
...
- Are the March 2026 LRs (lr_W=9e-4) still optimal?
