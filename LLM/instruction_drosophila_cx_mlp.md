# Drosophila CX — MLP Baseline Exploration
...
This serves as an **uninformed baseline** to compare against the GNN, which discovers ODE structure from data.
...
1. **Full Log (append-only)**: `drosophila_cx_mlp_Claude_analysis.md`
   - Append every iteration's log entry (4 entries per batch)
   - Never read — human record only

2. **Working Memory (read + update every batch)**: `drosophila_cx_mlp_Claude_memory.md`
...
Example: "lr=1e-3 with hidden_dim=256 achieves rollout_pearson > 0.6 robustly (3/3 iterations, all seeds > 0.55, CV < 5%)"
...
Example: "Does higher n_layers improve Jacobian structure? Only iter 1 tested with n_layers=5."
...
Example: "Hypothesis: 'Larger hidden_dim (512) improves Jacobian recovery over 256' — Falsified by iter 2 (hidden_dim=512 gave CV=8%, worse than 256 with CV=3%). Revised: 'MLP capacity has diminishing returns; 256 is sufficient.'"
...
```markdown
# Working Memory: drosophila_cx_mlp

## Paper Summary (update at every block boundary)

- **MLP baseline**: [pending]
