# Drosophila Larva — Known ODE Exploration (GT Edges, Noise 0.5)

## Goal

Maximize **connectivity_R2** (PRIMARY) for the **Drosophila larva two-population motor model** using the **known_ode model** with **GT edge topology** (4,222 edges) and **strong intrinsic noise (sigma=0.5)**.

The known_ode model uses the **exact activation function**: `g_phi = softplus(v + b)` and **exact dynamics**: `dv/dt = (-v + g * msg + I)`. All parameters are **learned from data**: W, g (gain per neuron), bias per neuron. This combines perfect structural knowledge with known topology and strong noise.

**Starting hypothesis**: "Known ODE + GT edges + noise=0.5 achieves reasonable W R2 (>0.5), benefiting from strong GT constraint"
...
At strong noise with GT edges and perfect structure, recovery is challenging. GT edges provide constraint but noise adds fundamental limits.
...
- **GT edges provide strong constraint but noise is limiting**: Even perfect topology may not overcome identifiability limits under overwhelming noise.
- **Strong regularization likely needed**: W_L2 and W_sign may become critical stabilizers.
...
Destination: `config/larva/larva_known_ode_gt_edges_noise_05_winner.yaml`

## File Structure

1. **Full Log (append-only)**: `larva_known_ode_gt_edges_noise_05_Claude_analysis.md`
2. **Working Memory**: `larva_known_ode_gt_edges_noise_05_Claude_memory.md`
...
- Hypothesis: "Known ODE + GT edges + noise=0.5 maintains reasonable connectivity_R2 (>0.5) robustness despite strong noise"
...
```markdown
# Working Memory: larva_known_ode_gt_edges_noise_05
