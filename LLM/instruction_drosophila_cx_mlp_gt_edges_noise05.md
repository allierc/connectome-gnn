# Drosophila CX — MLP Baseline + GT Edges + Noise 0.5 Exploration

## Goal

Maximize **rollout_pearson** (primary) and **connectivity_R2** (secondary) for the **MLP baseline** on the Drosophila central complex ring attractor with **GT edges topology + intrinsic noise σ=0.5**.
...
**Important**: The MLP itself does not use graph structure — `use_gt_edges: true` is set for pipeline consistency. This experiment serves as a **control baseline** against the GNN GT-edges + noise=0.5 experiment (GNN best: 0.999+). Prior MLP result on clean FC data: W R2 ≈ 0.
...
At high noise with GT edges, test whether strong noise helps or hinders MLP Jacobian extraction compared to GNN advantage.
...
- **Intrinsic noise σ=0.5** added during simulation
...
Destination: `config/drosophila_cx/drosophila_cx_mlp_gt_edges_noise05_winner.yaml`

## File Structure

You maintain THREE files:

1. **Full Log (append-only)**: `drosophila_cx_mlp_gt_edges_noise05_Claude_analysis.md`
   - Append every iteration's log entry (4 entries per batch)
   - Never read — human record only

2. **Working Memory (read + update every batch)**: `drosophila_cx_mlp_gt_edges_noise05_Claude_memory.md`
...
Example: "lr=1e-3 with hidden_dim=256 on GT edges + noise=0.5 achieves rollout_pearson > 0.65 (3/3 iterations, all seeds > 0.60, CV < 5%)"
...
Example: "Does MLP benefit more from high noise at reduced edge space? Only iter 1 tested."
...
Example: "Hypothesis: 'High noise enriches Jacobian activity covariance, improving W R2' — Falsified by iter 2 (noise=0.5 connectivity_R2 lower than expected). Revised: 'Noise magnitude dominates; structured W (GNN) needed for robust recovery.'"
...
- Hypothesis: "The MLP baseline with GT edges + noise=0.5 may show improved Jacobian extraction due to high noise enriching activity covariance, but W R2 remains fundamentally limited vs GNN"
...
```markdown
# Working Memory: drosophila_cx_mlp_gt_edges_noise05

## Paper Summary (update at every block boundary)

- **MLP baseline + GT edges + noise 0.5**: [pending]
