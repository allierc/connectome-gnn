...
At mild noise with GT edges, test whether MLP Jacobian extraction improves or remains limited.
...
1. **Full Log (append-only)**: `drosophila_cx_mlp_gt_edges_noise005_Claude_analysis.md`
   - Append every iteration's log entry (4 entries per batch)
   - Never read — human record only

2. **Working Memory (read + update every batch)**: `drosophila_cx_mlp_gt_edges_noise005_Claude_memory.md`
...
Example: "lr=1e-3 with hidden_dim=256 on GT edges + noise=0.05 achieves rollout_pearson > 0.60 (3/3 iterations, all seeds > 0.55, CV < 5%)"
...
Example: "Does noise benefit MLP Jacobian more than GNN at reduced edge space? Only iter 1 tested."
...
Example: "Hypothesis: 'Noise helps MLP Jacobian extraction via enriched covariance' — Falsified by iter 2 (noise=0.05 connectivity_R2 no better than clean). Revised: 'MLP Jacobian is fundamentally limited regardless of noise; structure (GNN/graph) is essential.'"
