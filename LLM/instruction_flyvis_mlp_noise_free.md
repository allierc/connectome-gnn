# FlyVis MLP Baseline — Rollout Optimization (noise-free)
...
This noise-free condition has already been extensively optimized. Key findings:

- **Best config**: hidden_dim=640, n_layers=2, relu, add_residual=true, lr=1e-5, DAL=100, bs=256, rollout_train_steps=1
- **Best RMSE**: 0.0799 +/- 0.0581, Pearson r=0.926 +/- 0.063
- **Robustness**: CV ~1.5% across 13+ seeds

Key established principles from prior optimization:
1. **n_layers=2 is optimal** — simpler architecture generalizes better in autoregressive mode (2L < 3L < 5L)
2. **add_residual is essential** — removing it causes catastrophic divergence
3. **rollout_train_steps=1 is optimal** — multi-step training hurts
4. **lr=5e-6 to 1e-5 is the sweet spot** — lower is too slow, higher causes instability
5. **hidden_dim scaling**: 640 > 512 > 384 > 256 > 128 (monotonic improvement with width)
6. **Training parameter space is exhausted** — the remaining gap to 1e-2 likely requires graph structure
