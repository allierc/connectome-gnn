...
At this clean noise level, known_ode optimization should be most tractable. The challenge is purely parameter estimation from perfect ODE dynamics.
...
1. **Full Log (append-only)**: `drosophila_cx_known_ode_Claude_analysis.md`
   - Append every iteration's log entry (4 entries per batch)
   - Never read — human record only

2. **Working Memory (read + update every batch)**: `drosophila_cx_known_ode_Claude_memory.md`
...
Example: "lr_W=1e-3 with lr=1e-3 achieves connectivity_R2 > 0.5 robustly on clean data (3/3 iterations, all seeds > 0.45, CV < 3%)"
...
Example: "Does higher W_L2 improve robustness? Only iter 2 tested with mixed results."
...
Example: "Hypothesis: 'Known ODE achieves connectivity_R2 > 0.7 with default parameters' — Falsified by iter 1 (best only 0.52). Revised: 'Known ODE baseline is 0.5; meaningful improvement requires careful hyperparameter tuning.'"
