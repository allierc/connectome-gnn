...
**Keep total training time <= 60 min per iteration.** Medium model (230 neurons). Typical: ~10-20 min/epoch. When increasing n_epochs, halve data_augmentation_loop to stay within budget.
...
| 3 | **Regularization** | `coeff_W_sign`, `coeff_W_L1`, `coeff_g_phi_diff`, `coeff_f_theta_msg_diff` | W_sign: {0, 0.05, 0.2}, W_L1: {0, 1e-5, 5e-5}, g_phi_diff: {500, 1000, 1500}, f_theta_msg_diff: {0, 10, 50} |
...
### What NOT to explore (established from prior CX exploration)

- `w_init_mode=randn` (zeros is consistently best)
...
### Model-specific notes for Block 3
