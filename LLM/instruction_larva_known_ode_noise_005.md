# Drosophila Larva — Known ODE Exploration (Noise 0.05)

## Goal

Maximize **connectivity_R2** (PRIMARY) for the **Drosophila larva two-population motor model** using the **known_ode model** under **intrinsic noise (sigma=0.05)**.
...
- **Noise=0.05 is mild**: From GNN experience, noise=0.05 improved connectivity R2 substantially. Known ODE should benefit similarly or more.
...
3. Save it to `config/larva/larva_known_ode_noise_005_winner.yaml` with a YAML comment header:

```yaml
# Winner config: larva_known_ode_noise_005_winner.yaml
...
Destination: `config/larva/larva_known_ode_noise_005_winner.yaml`
...
1. **Full Log (append-only)**: `larva_known_ode_noise_005_Claude_analysis.md`
   - Append every iteration's log entry (4 entries per batch)
   - Never read — human record only

2. **Working Memory (read + update every batch)**: `larva_known_ode_noise_005_Claude_memory.md`
...
```markdown
# Working Memory: larva_known_ode_noise_005
