...
Test **known_ode performance with combined noise sources** for the **Drosophila visual system** with dynamics noise 0.005 and measurement noise 0.010. The goal is to find a **combined-noise-robust config** that achieves **connectivity_R2 > 0.80 on ALL 4 seeds with CV < 4%**, demonstrating resilience to realistic multi-source noise.

**Primary objective**: Establish a **robust config** that achieves **connectivity_R2 > 0.80 across all 4 seeds with CV < 4%**, testing known_ode under realistic recording conditions where both the neural simulation and measurement contain noise.

**Why this matters**: known_ode is an inverse problem — we have the ODE dynamics `f(v, W)` known, and we learn connectivity W and biophysical parameters (tau_i, V_rest_i) directly from noisy neural observations. Combined noise sources (measurement noise + dynamics noise) present a more realistic challenge than single noise types, as they test robustness to multiple sources of signal corruption. This regime helps establish the practical operating conditions.
...
Given noisy voltage observations, the inverse problem is to recover:
...
With combined noise sources (dynamics noise 0.005 + measurement noise 0.010), the gradient signal contains contributions from two independent noise processes. This tests whether the model can disentangle the underlying parameters despite this more complex noise structure.
...
2. **Measurement noise** (`measurement_noise_level=0.010`): `v_obs(t) = v_clean(t) + epsilon_meas(t)`, epsilon_meas ~ N(0, 0.010)

Combined noise std ~ sqrt(0.005^2 + 0.010^2) ~ 0.0112 in observations.
...
**Trade-off**: Combined noise requires balanced regularization; too little allows overfitting, too much prevents convergence.
...
**Robustness criteria** (combined noise data):
...
**Hypothesis**: "Combined noise requires moderate learning rates. Optimal LR will achieve connectivity_R2 > 0.75 while maintaining stability across seeds"
...
**Expected outcome**: Identify LR range for combined noise regime.

### Block 2 (iter 13-24): W Regularization
**Hypothesis**: "Combined noise requires regularization to prevent overfitting to noise artifacts. Optimal regularization improves connectivity_R2."
...
- Slot 2: coeff_W_L1=5e-4, coeff_W_L2=3e-4, coeff_W_sign=5e-6 (increased)
- Slot 3: coeff_W_L1=1e-3, coeff_W_L2=5e-4, coeff_W_sign=1e-5 (strong)

**Expected outcome**: Validate regularization balance for combined noise.

### Block 3 (iter 25-36): W Initialization + LR
**Hypothesis**: "Initialization interacts with learning rate under combined noise. Test whether scaled initialization improves stability."
...
**Expected outcome**: Determine best initialization for combined noise.

### Block 4 (iter 37-48): Batch Size + LR
**Hypothesis**: "Larger batch sizes help average combined noise sources. Interact with LR to stabilize convergence."
...
**Expected outcome**: Quantify batch size effect under combined noise.
...
1. **Full Log (append-only)**: `flyvis_noise_005_010_known_ode_Claude_analysis.md`
   - Append every iteration's log entry (4 entries per batch)
   - Never read — human record only

2. **Working Memory (read + update every batch)**: `flyvis_noise_005_010_known_ode_Claude_memory.md`
...
Example: "batch_size=8 improves robustness under combined noise (3/3 iterations, CV < 3%, all seeds > 0.80)"
...
Example: "Does combined noise require different regularization than single-source noise? Preliminary evidence from iter 1, needs validation."
...
Example: "Hypothesis: 'Combined noise requires 2× regularization strength' — Falsified by iter 4 (coeff_W_L2=3e-4 → R2=0.68, unstable). Revised: 'Combined noise is optimally handled with balanced regularization; exceeding baseline by 2× causes convergence issues.'"
...
2. Copy config from `log/Claude_exploration/LLM_flyvis_noise_005_010_known_ode/config/iter_XXX_slot_YY.yaml`
3. Save to `config/fly/flyvis_noise_005_010_known_ode_winner.yaml` with header:

```yaml
# Winner config: flyvis_noise_005_010_known_ode_winner.yaml
...
- Read base config — note `noise_model_level=0.005, measurement_noise_level=0.010` (combined noise)
- Initialize 4 slot configs for Block 1 (learning rate sweep)
- **Initial hypothesis**: "Combined noise requires moderate learning rates for stability. Optimal LR will achieve connectivity_R2 > 0.75 while maintaining CV < 4%"
...
- **known_ode under combined noise**: [How known_ode performs with both dynamics and measurement noise, which parameters matter most, best connectivity_R2 achieved, comparison to single-noise regimes]
- **Robustness findings**: [How stable is the best config across seeds? Key insights about noise interaction in learning dynamics.]
- **Optimization dynamics**: [Convergence behavior, learning rate sensitivity, effect of regularization under combined noise]
