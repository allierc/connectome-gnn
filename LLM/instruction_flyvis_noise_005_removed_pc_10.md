...
Maximize **connectivity_R2** for the FlyVis connectome model with **10% of edges removed per column** (390,701 edges out of the full 434,112). The GNN must recover the dynamics of the full connectome using only the pruned graph — edges that were removed are completely absent from the GNN's edge_index.
...
python GNN_LLM.py -o generate_train_test_plot_Claude flyvis_noise_005_removed_pc_10 iterations=48 --cluster --resume
```

The pipeline auto-creates `config/fly/flyvis_noise_005_removed_pc_10_Claude_00.yaml` through `_03.yaml` on first run. **Do not create these files manually.**
...
- **390,701 edges** (10% of 434,112 real edges removed per column)
...
3. Save to `config/fly/flyvis_noise_005_removed_pc_10_winner.yaml` with header:

```yaml
# Winner config: flyvis_noise_005_removed_pc_10_winner.yaml
...
- Hypothesis: "The March 2026 working condition (conn_R2 > 0.9) is reproducible with 10% edges removed, all penalties=0, across 4 training seeds."
...
a section with header `## flyvis_noise_005_removed_pc_10 — Key Discoveries (YYYY-MM-DD)` containing
...
```markdown
# Working Memory: flyvis_noise_005_removed_pc_10
...
- Does removing 10% of edges measurably reduce conn_R2 vs baseline (0% removal)?
