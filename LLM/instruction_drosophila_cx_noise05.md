...
This exploration starts from the **best noise=0.05 config** and tests whether stronger noise further improves or degrades connectivity recovery. The noise dose-response is already established for flyvis (noise-free=0.926, noise005=0.985, noise05=0.990 — monotonically improving) and being tested for zebrafish (noise005=0.918). For CX, noise=0.05 achieved 0.972 best (0.845 mean over 6 seeds) — a major improvement over clean FC (0.804 best, 0.574 mean). The question is whether noise=0.5 pushes even higher or starts to overwhelm the ring attractor dynamics.
...
| `hidden_dim`              | 96      | MLP hidden dimension (from noise005 winner)            |
| `embedding_dim`           | 4       | Embedding dimension (from noise005 winner)             |
...
| `coeff_W_L2`              | 5e-5    | L2 penalty on W (from noise005 winner)                 |
| `coeff_W_sign`            | 0       | Dale's law penalty                                     |
| `use_gt_edges`            | false   | Fully connected graph (N^2-N = 22,952 edges)           |
| `dale_law`                | true    | **Enforce Dale's law** (from noise005 winner — critical stabilizer) |
...
| ----- | ------------------------------ | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| 1     | **Baseline validation**        | None (robustness test)                                                     | Run best noise005 config with noise=0.5 across 4 seeds. Does 10x more noise help or hurt?                       |
...
- **dale_law=true is established**: The noise005 exploration proved this is the single biggest lever. Keep it unless evidence clearly says otherwise.
- **f_theta_msg_diff=50 is established**: From noise005. May need adjustment at higher noise.
...
3. Save it to `config/drosophila_cx/drosophila_cx_noise05_winner.yaml` with a YAML comment header:
...
Destination: `config/drosophila_cx/drosophila_cx_noise05_winner.yaml`
...
- Read base config — the parent noise005 config + noise_model_level=0.5 IS the baseline.
...
```markdown
# Working Memory: drosophila_cx_noise05
