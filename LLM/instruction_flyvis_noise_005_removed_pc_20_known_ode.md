...
- DAVIS visual input, **noise_model_level=0.05 + 20% missing edges** (incomplete connectivity)
...
3. Save it to `config/fly/flyvis_noise_005_removed_pc_20_known_ode_winner.yaml` with a YAML comment header:
...
Destination: `config/fly/flyvis_noise_005_removed_pc_20_known_ode_winner.yaml`
...
- Hypothesis: "Known ODE with 20% missing edges can still achieve connectivity_R2 > 0.85 with appropriate regularization"
