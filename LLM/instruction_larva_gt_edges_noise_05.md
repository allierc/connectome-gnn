# Drosophila Larva GT Edges (Noise 0.5) — LLM Exploration

## Goal

Maximize **connectivity_R2** for the **Drosophila larva two-population motor model** (Beiran & Litwin-Kumar 2023, Figure 5a-c) using **ground-truth edges** under **strong intrinsic noise (sigma=0.5)**.

This exploration tests the limit of GT edges under heavy noise. Cross-model evidence shows noise=0.05 helps recovery, but noise=0.5 is 10× stronger. The question is whether GT edges' structural constraint still provides benefit under overwhelming noise, or if noise becomes a fundamental limit on identifiability.

Data is **re-generated each iteration** with a different seed to verify seed independence.

### Parent config (best GT edges from noise=0.05 exploration)

From prior GT edges + noise=0.05 exploration.
...
At high noise level, even GT edges may struggle. This regime tests the identifiability limit of the larva dynamics under heavy process noise.
...
| ----- | ------------------------------ | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| 1     | **Baseline validation**        | None (robustness test)                                                     | Run best GT edges noise=0.05 config with noise=0.5 across 4 seeds. Does it degrade or improve? |
| 2     | **Regularization re-tune**     | `coeff_W_L1`, `coeff_W_L2`, `coeff_W_sign`                                | W_L1: {0, 1e-6, 5e-6}, W_L2: {1e-5, 5e-5, 1e-4}, W_sign: {0, 0.1, 0.5}. Heavy noise requires strong regularization. |
| 3     | **Training volume re-tune**    | `data_augmentation_loop`, `n_epochs`                                       | DAL: {100, 200, 500}, n_epochs: {2, 4, 8}. Very high noise may need extensive training. |
| 4     | **Architecture + batch_size**  | `hidden_dim`, `embedding_dim`, `batch_size`                                | hidden_dim: {64, 96, 128}, embedding_dim: {2, 4}, batch_size: {2, 4, 8}. Larger models may help under heavy noise. |
| 5     | **Monotonicity + Dale's law**  | `coeff_g_phi_diff`, `coeff_f_theta_diff`, `coeff_f_theta_msg_diff`, `dale_law` | g_phi_diff: {1500, 2000, 3000}, f_theta_diff: {0, 50, 100}, dale_law: {false, true}. Strong constraints may be necessary. |
...
- **10× stronger noise than noise=0.05**: Expected performance degradation. The focus is on finding which regularizers/constraints help the most.
- **May hit identifiability ceiling**: Very strong noise could make W unrecoverable regardless of hyperparameters.
...
3. Save it to `config/larva/larva_gt_edges_noise_05_winner.yaml` with a YAML comment header:

```yaml
# Winner config: larva_gt_edges_noise_05_winner.yaml
...
Destination: `config/larva/larva_gt_edges_noise_05_winner.yaml`
...
1. **Full Log (append-only)**: `larva_gt_edges_noise_05_Claude_analysis.md`
   - Append every iteration's log entry (4 entries per batch)
   - Never read — human record only

2. **Working Memory (read + update every batch)**: `larva_gt_edges_noise_05_Claude_memory.md`
...
- Read base config — the parent GT edges noise=0.05 config + noise_model_level=0.5 IS the baseline.
- Block 1 is a **robustness test**: all 4 slots use the same config (different seeds).
- Hypothesis: "GT edges with noise=0.5 maintains reasonable connectivity_R2 robustness, though degraded from noise=0.05"
...
```markdown
# Working Memory: larva_gt_edges_noise_05
