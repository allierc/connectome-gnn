...
\item \textbf{$E_i$ anchored on the learned per-type bias}, sigmoid band, excursion $1.0$:
      $E_i = \mathrm{bias}_i + \mathrm{band}(\cdot)$, band
      $[1.5, 6.0]$ excitatory (1 free variable), $[-6.0, -0.5]$ inhibitory (65 free)
      $\Rightarrow$ \textbf{66 free variables, 130 distinct reversals} --- the anchor is
      per type, so $E_{\mathrm{exc}}$ takes 65 values too
