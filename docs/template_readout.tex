...
\paragraph{The generator's equation, rewritten for the model.} Nothing in the MLP
$f_\theta$ is a time constant or a resting potential, so rewrite \eqref{eq:gen}
in the shape \eqref{eq:model} could take --- one bracket, one prefactor --- with
its constants left open:
%
\begin{equation}
...
\end{equation}
%
$T_i$, $V_i$, $G_i$ and $f(\cdot)$ are not claims about the model: they are four
numbers \S\ref{sec:update} obtains by least squares. Three have counterparts in
\eqref{eq:gen} --- $1/\tau_i$, the constant level $V^{\mathrm{rest}}_i$ (not the
moving state $\vi$), and $f$ linear on this data. $G_i$ has none, because there
the message enters the bracket with a coefficient of exactly $1$.
