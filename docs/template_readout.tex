...
\paragraph{Describing the model in the generator's terms.} $f_\theta$ is an MLP:
there is no $\tau$ in it to read off, no resting potential, no message gain. What
there is, on this data, is a function close to affine in its inputs --- and any
affine function of $(\vi, \widehat{\msg}_i, I_i)$ can be grouped into the shape of
\eqref{eq:gen}:
%
\begin{equation}
...
\end{equation}
%
So $T_i$, $V_i$, $G_i$ and $f(\cdot)$ are not assumptions about the model, they
are four numbers measured from it: \S\ref{sec:update} regresses the model's own
$d\vi/dt$ on $(1, \vi, \widehat{\msg}_i, I_i)$ and regroups the coefficients,
reporting the $R^2$ that says how well ``close to affine'' held. Three have
counterparts in \eqref{eq:gen} --- $1/\tau_i$, the constant level
$V^{\mathrm{rest}}_i$ (not the moving state $\vi$), and $f$ linear on this data.
$G_i$ has none: there the message enters the bracket with a coefficient of
exactly $1$.
