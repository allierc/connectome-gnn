...
\paragraph{The gauge cancels.} Substituting the gauge \eqref{eq:gauge} into the
per-edge form \eqref{eq:edgelin} divides the driving-force term by $k_i$ but
leaves its shape untouched, so the fitted coefficients become
%
\begin{equation}
...
-\frac{b_1}{b_2} = \Eij \ \ \text{exactly}.
\label{eq:Egauge}
\end{equation}
%
$\Wij$ is a coefficient and inherits the $1/k_i$, which is why \eqref{eq:wcorr}
multiplies it back; $\Eij$ is a \emph{ratio} of two coefficients that carry the
same factor, so $k_i$ divides out. The offset $\beta_i$ cannot reach it either:
it is absorbed by the third column. This is the precise content of ``$\Eij$
needs neither correction''.

\paragraph{But $\Eij$ is exposed to a contamination $\Wij$ is not.} The gauge
\eqref{eq:gauge} is stated per \emph{neuron}, and nothing in it constrains how
the model splits that message across the edges feeding $i$. Suppose the model's
per-edge message carries, besides the driving-force term, a component
proportional to the drive with no driving force in it:
%
\begin{equation}
