...
\paragraph{The redundancy.} Rescale and shift a neuron's message, and change
$f_\theta$'s sensitivity to it by the reciprocal amount:
%
\begin{equation}
\widehat{\msg}_i \;\longmapsto\; \frac{1}{c}\,\widehat{\msg}_i + d,
\qquad
\frac{\partial f_\theta}{\partial \widehat{\msg}_i} \;\longmapsto\;
   c\,\frac{\partial f_\theta}{\partial \widehat{\msg}_i},
\qquad
V_i \;\longmapsto\; V_i - G_i d ,
\label{eq:gaugetransform}
\end{equation}
%
and $d\vi/dt$ is \emph{identically} unchanged --- same trajectory, same loss,
same everything the training ever sees. A whole two-parameter family of
$(\widehat{\msg}, f_\theta)$ pairs describes one and the same network. That
family is the gauge freedom of this parameterisation: a redundancy in the
description, not a property of the circuit. Training cannot prefer one member of
it over another, because the objective is constant along it.

\paragraph{The ansatz, which is not the gauge.} Equation \eqref{eq:ansatz} below
is a different object and it is worth keeping the two apart. It is the
\emph{supposition} that the trained model landed somewhere on the orbit of the
truth --- that its message differs from the generator's only by the redundancy
\eqref{eq:gaugetransform} and not by anything else:
%
\begin{equation}
...
\end{equation}
%
$(k_i, \beta_i)$ are then the \emph{gauge parameters}: the coordinates saying
which member of the family this particular trained model ended up as. They are
what the readout measures and undoes. The ansatz can be false --- a model whose
message is off the orbit is one whose per-edge form is not the generator's at
all, and \S\ref{sec:Ecorr} is about exactly that case --- so it is tested, by
the per-edge $R^2$ of \S\ref{sec:readout}, rather than assumed.

There are two ways to deal with the redundancy, and this project uses both.
\emph{Fixing} the gauge during training adds a term that makes one member the
only minimiser: \texttt{coeff\_f\_theta\_msg\_gain} drives $k_i \to 1$ and
\texttt{coeff\_g\_phi\_silent} drives $\beta_i \to 0$. \emph{Undoing} it after
training, which is what this document is about, leaves the model wherever it
landed and measures $(k_i, \beta_i)$ instead.
