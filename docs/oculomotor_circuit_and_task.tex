...
\textbf{From the mechanics to a transfer function.} Step 7 is a linear differential equation, so take its Laplace transform. Writing $X(s)$ for the transform of the gaze $\mathbf{x}(t)$ and $X_\infty(s)$ for that of the commanded equilibrium, and starting from rest so the initial-condition terms vanish, $\dot{\mathbf{x}}\mapsto sX$ and $\ddot{\mathbf{x}}\mapsto s^{2}X$, so
...
\textbf{First: it does not shrink the target.} Put $s=0$. Every term with an $s$ in it disappears and what is left is $H(0)=K^{-1}K=I$ --- exactly one, not approximately.
...
That is a stronger statement than it sounds, and it is what makes the two traces \emph{equal} rather than merely proportional. Suppose $H(0)$ were $0.9$ instead. Then every held gaze would fall a tenth short of its command, red would sit consistently outside blue, and the gap would grow with eccentricity --- a visible fan opening toward the edges of the workspace, and a gain the controller would have to learn to compensate. None of that happens, and none of it can: $H(0)=I$ is forced by the form of the equation, because the same $K$ that multiplies $\mathbf{x}$ on the left multiplies $\mathbf{x}_\infty$ on the right. The eye was built that way in step 7 precisely so that the static map means what it says.

\textbf{Second: it delays it.} Since $H$ is not the identity at nonzero $s$, ask what it \emph{is} for small $s$. Factor $K$ out:
...
the last step because $e^{-\Delta s}=I-\Delta s+O(s^{2})$ has the same first-order term. Multiplying by $e^{-\Delta s}$ in the frequency domain \emph{is} shifting by $\Delta$ in time, so to first order the eye returns its command unchanged, $\Delta$ later.

\textbf{The assumption, stated.} This is a \textbf{low-frequency} expansion and nothing else: it needs $\lVert K^{-1}Cs\rVert$ and $\lVert K^{-1}s^{2}\rVert$ both small, which for $s=i\omega$ means
...
with $\Delta=K^{-1}C$ predicting 127 ms in $\theta$, 118 in $\varphi$ and 137 in $\psi$ --- the low-frequency rows to within a millisecond.
