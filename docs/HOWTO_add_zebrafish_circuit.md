...
|---|------|---------------|-------------------|-----|
| (i) | **Loader category map** | `connectome_loaders._ZHD_AFFERENT_PREFIXES` ([:296](../src/connectome_gnn/generators/connectome_loaders.py#L296)) and `_zhd_category()` (returns `""` for unknown → row rejected) | a `HNd` row is dropped, not loaded | add `"HNd"` to the afferent prefixes + category map; build its `HNd_L/R` subpop in the `afferent_subpop_ix` section (~[:519](../src/connectome_gnn/generators/connectome_loaders.py#L519)) alongside the RIPN→PENa / pt-IPN→PENb split |
