...
\noindent\textbf{Note:} The noise-free and noise-0.05 rows are identical, suggesting the noise level $\sigma = 0.05$ is too small to noticeably affect the per-neuron singular value spectrum. This requires verification: the data loading and noise injection should be checked to ensure different noise levels are being analyzed. The noise-0.5 data clearly shows the effect of stronger noise: mean rank increases to 26.7, shrinking the null space by 65\%.
