...
Effective rank at 99\% variance (noise-free) & \textbf{45} \\
Effective rank at 99\% variance (noise-0.05) & \textbf{134} \\
Effective rank at 99\% variance (noise-0.5) & 781 \\
\bottomrule
\end{tabular}
\end{center}
...
Summing over all 13,697 postsynaptic neurons gives the global null space estimate. At the 99\% threshold ($r_{\text{global}} = 400$), this exceeds the maximum in-degree (208), so all neurons are fully identifiable:

\begin{center}
\begin{tabular}{lrl}
\toprule
In-degree range & Neurons & Null space contribution at rank 400 \\
\midrule
1--10   & 3,182 & 0 (fully identifiable) \\
11--20  & 3,917 & 0 (fully identifiable) \\
21--45  & 3,196 & 0 (fully identifiable) \\
46--100 & 2,843 & 0 (fully identifiable) \\
101--208 & 559  & 0 (fully identifiable) \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
noise-free & 45 & 115,223 & \textbf{26.5\%} \\
noise-0.05 & 134 & 10,227 & \textbf{2.4\%} \\
noise-0.5 & 781 & 0 & \textbf{0.0\%} \\
\bottomrule
\end{tabular}
\end{center}

\noindent\textbf{Breakdown across variance thresholds (noise-free):}
...
\noindent\textbf{Interpretation:} The global rank reveals a clear monotonic trend: as noise increases, the effective rank of the population activity rises (45 → 134 → 781), shrinking the null space and improving identifiability (115,223 → 10,227 → 0). At the noise-free threshold, the global rank 45 is lower than the mean per-neuron rank (17.5), which appears counterintuitive but reflects that the global rank measures the rank of the \emph{full population}, whereas per-neuron rank measures individual neurons' activity matrices---neurons with high in-degree can have lower individual ranks due to correlated inputs. The global SVD provides an \emph{upper bound} on each neuron's rank assuming the global correlations apply uniformly, but this bound is tight for the noise-free case.
...
\midrule
noise-free & 6.2 & 348,545 & \textbf{80.3\%} \\
noise-0.05 & 11.1 & 282,273 & \textbf{65.0\%} \\
noise-0.5 & 26.7 & 69,047 & \textbf{15.9\%} \\
\bottomrule
\end{tabular}
\end{center}

\noindent\textbf{Note:} These results use training data (x\_list\_train/voltage.zarr, 64,000 frames subsampled to 8,000), which is 7.5× larger than test data and reveals much stronger degeneracy. The mean per-neuron effective rank is very low (6.2 for noise-free), indicating strong within-type correlations at the individual neuron level. Noise dramatically reduces degeneracy: rank increases 6.2 → 11.1 → 26.7 as $\sigma$ increases, shrinking null space from 80.3\% to 15.9\%.

The per-neuron null space exceeds the global SVD bound (115,223) by a factor of 1.2--2.7, indicating that per-neuron analysis reveals stronger degeneracy than the population-level upper bound. Noise at $\sigma = 0.5$ monotonically increases the effective rank per neuron, reducing the null space and improving identifiability: in noise-free data, $r_i$ averages $17.5 \pm 18.4$, but with added process noise (noise-0.5), the rank increases significantly, shrinking the null space.
...
The three approaches measure degeneracy at different scales and data regimes. At the 99\% variance threshold:
...
\midrule
Global SVD & 115,223 & \textbf{26.5\%} \\
Per-neuron SVD & 348,545 & \textbf{80.3\%} \\
Structural per-type & 121,100 & \textbf{27.9\%} \\
\bottomrule
\end{tabular}
\end{center}

\noindent\textbf{Why the estimates diverge.} The three methods reveal different aspects of the degeneracy:

\begin{itemize}[nosep]
\item \textbf{Global SVD (26.5\%):} Computes the rank of the full $8000 \times 13741$ population activity matrix by subsampling. This coarse bound assumes the population-level rank applies uniformly to all neurons, missing strong within-neuron correlations.

\item \textbf{Per-neuron SVD (80.3\%):} Measures the rank of each individual neuron's $8000 \times d_i$ activity matrix $\mathbf{H}_i$. Mean per-neuron rank is very low ($6.2$) despite in-degrees up to 208, revealing that individual neurons see highly correlated presynaptic activity. This is the \emph{most direct empirical measurement} of true degeneracy.

\item \textbf{Structural per-type (27.9\%):} Counts exact null dimensions from connectome structure (same-type edges with $k>1$ contributing $k-1$ dimensions). This mechanistic bound explains the within-type portion but misses cross-type correlations.
\end{itemize}

The per-neuron SVD (80.3%) is substantially higher than both the global SVD and structural counts because it reveals that individual neurons experience much stronger input correlations than the population-level analysis suggests.
...
\item The effective rank of $\mathbf{H}_i$ increases from 17.5 (noise-free, mean per-neuron rank) toward $d_i$
...
Recovering the connectome from neural dynamics is fundamentally ill-posed: the columnar organization of the flyvis circuit forces same-type neurons across columns to produce strongly correlated activity, making their individual synaptic contributions indistinguishable. Three independent estimates bracket the true degeneracy: (1) global SVD bound (115,223 dimensions, 26.5\%); (2) per-neuron SVD empirical measurement (193,824 dimensions, 44.6\%); and (3) structural per-type count (121,100 dimensions, 27.9\%). All three agree the null space is substantial (23--45\%). This structural degeneracy affects between 28\%--45\% of all 434,112 edge weights. This is supported empirically by generating 780 perturbed connectivity matrices where weights change measurably (conn.\ $R^2$ ranging from 0.28 to 1.0, most between 0.93 and 0.99) while dynamics remain nearly identical (rollout $R^2 > 0.96$). Adding independent process noise breaks the within-type correlations and increases the effective activity rank from 45 (noise-free global bound) to 781 ($\sigma = 0.5$), eliminating the null space entirely and enabling the GNN to achieve $R^2_{\mathbf{W}} = 0.997$. The entire analysis rests on the rank-nullity theorem: a neuron's incoming weights are recoverable when the activity rank exceeds its in-degree. The per-neuron SVD directly measures this; the global SVD provides an upper bound. \textbf{The condition $r \geq d_i$ provides a principled, per-neuron criterion for when connectome recovery from dynamics is well-posed or ill-posed.}
