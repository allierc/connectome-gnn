...
\textbf{The inverse problem}: given observed trajectories $\{v_i(t)\}_{t=0}^{T}$, stimuli $\{I_i(t)\}$, and the binary adjacency matrix $\mathbf{A}$ (which edges exist), recover the connectivity weights $\mathbf{W}$, time constants $\tau$, and resting potentials $V^{\text{rest}}$.
...
This equation holds independently for each postsynaptic neuron $i = 1, \ldots, N_{\text{post}}$, where $N_{\text{post}} = 13{,}697$ and $d_i$ ranges from 1 to 208 (the in-degree distribution of the flyvis connectome).
...
\noindent\textbf{Global bound from SVD.} An SVD of the full population activity reveals that the entire circuit's activity lives in a low-dimensional subspace. The computation subsamples to 982 neurons (every 14th of 13,741) and 8,000 timesteps (every 8th of 64,000), then computes the full SVD (all 982 singular values):
...
\noindent\textbf{Sensitivity to the variance threshold.} Since the 99\% threshold is arbitrarily chosen, we report the null space estimate across several thresholds:
...
\end{itemize}
\noindent\textbf{Protocol.} For each of the 52 non-retina types, we generated sum-preserving random perturbations at 15 scales (from $\lambda = 0.05$ to $8.0$), producing 780 degenerate connectivity matrices. Each was rolled out through the full ODE from identical initial conditions and stimulus.
