...
\title{\vspace{-1cm}\textbf{\color{quantablue}How Graph Neural Networks Are Learning\\[2pt] to Reverse-Engineer Small Brains' Wiring}}
\author{%
...
\section{The Inverse Problem of Small Brains}
...
\section{Scaling to the Fly's Small Brain}
...
\section*{\color{quantablue}The Hidden Dimensions of Neural Circuits}

\vspace{0.2cm}

\noindent There is a structural analogy between the situation facing this research program and the one facing string theory. In physics, general relativity operates in four dimensions---three of space and one of time---and describes gravity with extraordinary precision at all accessible scales. String theory proposes that reality has ten dimensions: the familiar four, plus six extra spatial dimensions curled up at the Planck scale ($\sim\!10^{-35}$\,m), far too small to observe directly. The four-dimensional theory is the \emph{effective} theory that emerges after these small dimensions are compactified: it captures the low-energy physics faithfully, but it cannot represent the degrees of freedom that were integrated out.

For three years, this research program has operated in a directly analogous regime. The state of each neuron is described by a single scalar: the membrane voltage $v_i(t) \in \mathbb{R}$. But a real neuron is not a single number. Its full dynamical state includes at least four distinct classes of hidden variables:
\begin{enumerate}[leftmargin=*,itemsep=1pt]
\item \textbf{Receptor-specific currents.} Each synapse carries multiple receptor types---AMPA ($\tau \sim 1$\,ms), NMDA ($\tau \sim 50$\,ms), GABA ($\tau \sim 10$\,ms)---each with its own kinetics, reversal potential, and voltage dependence. The scalar ODE collapses all of these into a single summed input $\sum_j \mathbf{W}_{ij}\,\text{ReLU}(v_j)$.
\item \textbf{Temporal history.} Axonal conduction delays, synaptic facilitation, and short-term depression mean that the message neuron $j$ sends to neuron $i$ depends not on the current voltage $v_j(t)$ alone, but on the trajectory $\mathbf{v}_j(t) = [v_j(t), v_j(t{-}1), \ldots]^\top$. The scalar ODE sees only $v_j(t)$.
\item \textbf{Adaptation and intrinsic excitability.} Calcium-activated potassium channels, spike-frequency adaptation, and after-hyperpolarization create neuron-internal memory that modulates the response to incoming drive. These are absent from the leak-integrator equation.
\item \textbf{Neuromodulatory state.} Dopamine, serotonin, acetylcholine, and norepinephrine reconfigure effective synaptic weights on timescales of seconds to minutes. A single real-valued $\mathbf{W}_{ij}$ cannot represent a synapse whose sign and strength depend on context.
\end{enumerate}

These hidden variables are the ``extra dimensions'' of neural dynamics. They are real and measurable in principle---with patch clamp, optogenetics, or pharmacology---but \emph{invisible to the instruments that generate large-scale datasets}. Calcium imaging, the dominant recording technology, reports a slow ($\sim$100\,ms decay), nonlinear, non-negative proxy: a convolution of the true voltage with an indicator kernel, sampled at video frame rates. Neuropixels electrodes are faster but sparse. In both cases, the observation is a low-dimensional projection of a high-dimensional reality---exactly as the four-dimensional physics we measure is a projection of string theory's ten.

The compactification is not metaphorical. When a calcium indicator integrates a burst of fast synaptic currents ($\sim$1\,ms AMPA, $\sim$50\,ms NMDA, $\sim$10\,ms GABA) into a single slow fluorescence transient, it is performing precisely the operation that compactifies a small dimension: it rolls up the fast temporal structure into a single observable, erasing the information about \emph{which} receptor types fired in \emph{which} sequence. The scalar-voltage ODE used in this work---$\tau_i \dot{v}_i = -v_i + V_i^{\text{rest}} + \sum_j \mathbf{W}_{ij}\,\text{ReLU}(v_j) + I_i$---is the effective field theory that emerges after this compactification. It captures the slow dynamics faithfully, but it cannot represent the fast degrees of freedom that were integrated out.

What would the ``extra dimensions'' of neural dynamics look like? Two candidates are already taking shape. The first is \textbf{temporal depth}: replacing the scalar voltage $v_j(t)$ with a history vector $\mathbf{v}_j(t) \in \mathbb{R}^T$ of past activity, and introducing learned temporal filters $h_\xi(\mathbf{a}_i, \mathbf{v}_j)$ that compress the sender's history in a receiver-dependent way---generalizing axonal delay kernels into a structure that maps onto the transformer's Query-Key-Value architecture, with the sparse connectome $\widehat{\mathbf{W}}_{ij}$ replacing the attention mask. The second is \textbf{complex-valued connectivity}: promoting $\mathbf{W}_{ij} \in \mathbb{R}$ to $W_{ij}^{\mathbb{C}} = |W_{ij}|\,e^{i\varphi_{ij}} \in \mathbb{C}$, so that a single neuromodulatory parameter $\alpha$ can reconfigure all $N^2$ synapses simultaneously via $W_{ij}^{\text{eff}}(\alpha) = |W_{ij}|\cos(\varphi_{ij} - \alpha)$. This rescues Feynman-style interference at the message-passing step---synaptic signals can constructively and destructively combine depending on their phase alignment---and yields falsifiable predictions: two behavioral contexts determine all others, and antipodal contexts produce sign-reversed connectivity.

Neither extension has been tested against real data. Like string theory's extra dimensions, they are motivated by mathematical structure and biological plausibility rather than direct observation. The hidden variables---axonal delays, receptor-specific time constants, neuromodulatory phases---are real, measurable in principle, but invisible to the recording modalities that generate the datasets on which the GNN is trained. The question that will determine the next phase of this research is the same question that haunts string theory: do the compactified dimensions leave signatures in the low-energy observables? If the fast synaptic structure imprints itself on the slow calcium dynamics---through timing-dependent motifs, stimulus-specific gain changes, or context-dependent connectivity patterns that a scalar ODE cannot explain---then the higher-dimensional theory makes predictions that the four-dimensional one does not. If it does not, the extra dimensions may be mathematically coherent but experimentally inert: beautiful, and invisible.
