...
### Step 4: Choose Next Config

Select parent node: choose the iteration with **highest connectivity_R2** as parent. Use `parent=root` only for the very first iteration or after a full block reset.

**CRITICAL**: The `parent=P` in the Node line must be the **node ID** (integer) of the selected parent. Example: if you select node 3 as parent, write `Node: id=4, parent=3`.

Choose strategy:

| Condition                            | Strategy             | Action                                       |
| ------------------------------------ | -------------------- | -------------------------------------------- |
| Default                              | **exploit**          | Best R² node, try mutation                   |
...
You \***\*MUST\*\*** use the Edit tool to add/modify exploration rules in this file.
...
Next: parent=137; try lr=8E-5 to explore higher lr
