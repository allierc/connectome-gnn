...
$-20\%$ edges removed & $0.05$ & 347\,290
  & \comment{pending} & \comment{pending} & \comment{pending} & \comment{pending} & \comment{pending} & \comment{pending} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Five-fold cross-validation results for the \textbf{Known-ODE baseline} across noise levels
(mean~$\pm$~SD, $n{=}5$ seeds).
The ODE form and ReLU activation are fixed to Eq.~\ref{eq:simulated_update}; learnable
parameters are $\hat{\tau}_i$, $\hat{V}_i^{\mathrm{rest}}$, and $\widehat{W}_{ij}$ only
(no MLP modules, no latent embeddings).
Prediction: model re-trained on each YouTube-VOS fold and tested on that same fold
(no pre-trained DAVIS model available; zero-shot transfer columns not applicable).
Parameter recovery columns as in Table~\ref{tab:cv_results}.
\textcolor{green!50!black}{Green}: value $>0.9$.}
