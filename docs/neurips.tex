...
research summary, and proposed next intervention on the simulation and/or machine learning parameters. The primary optimization target is either identifiability of the connectivity weights or rollout prediction accuracy. The agent is instructed to perform scientific reasoning: testable hypotheses must be drawn, repeatable experiments are conducted to validate or falsify them. The agent is told not to predict the outcome of a model training. Only training results can validate or falsify its hypotheses. 
...
\begin{table}[h]
\centering
\caption{The agentic loop tuned the GNN training parameters specifically for each
noise level and connectivity setting.
The GNN evaluation results are given over five-fold cross-validation
(mean~$\pm$~SD, $n{=}5$ seeds).
Prediction: DAVIS-trained model tested on held-out YouTube-VOS folds;
one-step and rollout Pearson~$r$ over 8\,000 test frames.
Parameter recovery: model re-trained on five folds; $R^2_{\widehat{W}}$ over all edges,
$R^2_{\tau}$ and $R^2_{V^{\mathrm{rest}}}$ over $13\,741$ neurons,
...
\begin{table}[h]
\centering
\caption{A single GNN training parameter set tuned by the agentic loop on the
$+400\%$ null-edges condition was applied unchanged to all five conditions.
Prediction: model trained on $+400\%$ null-edges tested on held-out
YouTube-VOS folds. Parameter recovery: model re-trained on five folds with $+400\%$ training parameters. Gaps relative to Table~\ref{tab:cv_per_condition} reflect the effect of not
adapting the GNN training parameters to each condition.}
...
\begin{table}[h]
\centering
\caption{The agentic loop tuned the Known-ODE training parameters specifically for
each noise level and connectivity setting
(mean~$\pm$~SD, $n{=}5$ seeds).
The Known-ODE baseline fixes the ODE form and ReLU activation to
Eq.~\ref{eq:simulated_update}; only $\hat{\tau}_i$, $\hat{V}_i^{\mathrm{rest}}$, and
$\widehat{W}_{ij}$ are learned.
No DAVIS model is trained; prediction (re-train): model re-trained on each
YouTube-VOS fold and tested within that same fold.
Parameter recovery: model re-trained on five folds; $R^2_{\widehat{W}}$ over all
edges, $R^2_{\tau}$ and $R^2_{V^{\mathrm{rest}}}$ over $13\,741$ neurons,
GMM clustering accuracy over $65$ cell types.
\textcolor{green!50!black}{Green}: value $>0.9$.}
...
\begin{tabular}{lccrrrrrr}
\toprule
& & & \multicolumn{2}{c}{prediction (re-train)} & \multicolumn{4}{c}{parameter recovery} \\
\cmidrule(lr){4-5}\cmidrule(lr){6-9}
condition & $\sigma$ & edges
  & one-step $r$ & rollout $r$
  & $R^2_{\widehat{W}}$ & $R^2_{\tau}$ & $R^2_{V^{\mathrm{rest}}}$ & cluster\ acc. \\
...
noise $0.05$ + meas.\ $0.10$   & $0.05$ & $434\,112$
...
\begin{table}[h]
\centering
\caption{The agentic loop tuned the GNN+INR training parameters for the joint
GNN+SIREN model (mean~$\pm$~SD, $n{=}5$ seeds).
The SIREN implicit neural representation (INR) is trained jointly with the GNN
to reconstruct the visual stimuli.
Prediction (DAVIS): DAVIS-trained model tested zero-shot on held-out YouTube-VOS
folds; one-step and rollout Pearson~$r$ over $8{,}000$ test frames.
Prediction (re-train): GNN re-trained on each YouTube-VOS fold and tested
within that fold.
Stimuli $r$: Pearson~$r$ between INR-predicted and true visual stimuli (slope-corrected).
Parameter recovery: model re-trained on five folds;
$R^2_{\widehat{W}}$ over all edges,
$R^2_{\tau}$ and $R^2_{V^{\mathrm{rest}}}$ over $13{,}741$ neurons,
GMM clustering accuracy over $65$ cell types.
\textcolor{green!50!black}{Green}: value $>0.9$.}
\label{tab:cv_inr}
...
condition & $\sigma$ & edges
  ...
  & $R^2_{\widehat{W}}$ & $R^2_{\tau}$ & $R^2_{V^{\mathrm{rest}}}$ & cluster\ acc. \\
\midrule
noise $0.05$ (DAVIS)    & $0.05$ & $434\,112$
  & \comment{pending} & \comment{pending} & \comment{pending}
  & \comment{pending} & \comment{pending} & \comment{pending} & \comment{pending} \\
noise $0.05$ (re-train) & $0.05$ & $434\,112$
