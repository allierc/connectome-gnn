...
python run_janelia_cx_teacher.py --smoke

# Full Hulse-spec training (200k trials x 10 epochs, ~1 h on A100).
python run_janelia_cx_teacher.py \
    --output papers/janelia_cx/trained/janelia_cx_seed0.pt \
    --device cuda

# Short test (~5 min on a recent GPU) to confirm convergence trajectory.
python run_janelia_cx_teacher.py \
    --n_trials 5000 --n_epochs 3 \
    --output papers/janelia_cx/trained/janelia_cx_short.pt \
    --device cuda
...
from connectome_gnn.teachers.janelia_cx_teacher import _main  # noqa: E402
