...
def plot_synaptic(config, epoch_list, log_dir, logger, cc, style, extended, device, log_file=None, skip_svd=False):
    ...
    if epoch_list[0] != 'all':
        ...
        for epoch in epoch_list:
            ...
            # Local linearization of f_theta at per-neuron median voltage:
            #   tau   = -1 / slope_local              (improve_tau_extraction.py: +~0.10 R² over chord)
            #   Vrest = median_v - f_theta(median_v) / slope_local
            #                                         (improve_vrest_extraction.py: +~0.06 R² over chord)
            median_v = torch.median(activity[:n_neurons], dim=1).values.detach()
            v_pts = median_v.clone().to(device).requires_grad_(True)
            a_slice = model.a[:n_neurons]
            _tau_chunk = 2000
            local_slopes = torch.empty(n_neurons, device=device)
            f_theta_at_med = torch.empty(n_neurons, device=device)
            for _i in range(0, n_neurons, _tau_chunk):
                v_c = v_pts[_i:_i + _tau_chunk].reshape(-1, 1)
                a_c = a_slice[_i:_i + _tau_chunk]
                feats = _build_f_theta_features(v_c, a_c)
                y = model.f_theta(feats.float()).squeeze(-1)
                g, = torch.autograd.grad(y.sum(), v_pts, retain_graph=False, create_graph=False)
                local_slopes[_i:_i + _tau_chunk] = g[_i:_i + _tau_chunk]
                f_theta_at_med[_i:_i + _tau_chunk] = y.detach()
            local_slopes_np = to_numpy(local_slopes.detach())
            f_theta_at_med_np = to_numpy(f_theta_at_med)
            median_v_np = to_numpy(median_v)
            learned_tau = ode_params.derive_tau(local_slopes_np, n_neurons)
            r_squared_tau = 0.0
            slope_tau = 0.0
            ...
            gt_vrest_np = ode_params.gt_vrest(n_neurons)
            # V_rest from tangent-intercept at median voltage (reuses local_slopes + f_theta_at_med
            # computed for tau above). Falls back to median_v where slope is near zero.
            with np.errstate(divide='ignore', invalid='ignore'):
                learned_V_rest = np.where(
                    np.abs(local_slopes_np[:n_neurons]) > 1e-8,
                    median_v_np[:n_neurons] - f_theta_at_med_np[:n_neurons] / local_slopes_np[:n_neurons],
                    median_v_np[:n_neurons],
                )
